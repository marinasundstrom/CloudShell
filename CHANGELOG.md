...
- Added a NuGet package workflow and an explicit package solution. The workflow
  ...
  preview version to the public CloudShell MyGet feed, or a stable version to
  NuGet.org through Trusted Publishing, and then verifies a clean consumer
...
- Fixed C# launcher template serialization so provider-registered capability
  attribute paths are used when emitting resource templates, preserving
  root-level `environmentVariables` output for environment-variable-capable
  resources.
...
- Moved generated resource-view component parameter projection behind the same
  Resource Manager boundary and limited provider-contributed replacement views
  to the documented resource ID context.
...
- Changed graph-backed endpoint-mapping reconciliation to fail closed for
  ...
  dispatch.
- Changed graph-backed volume provisioning to fail closed for
...
- Changed graph-backed Secrets Vault inspection to fail closed when no Secrets
  ...
  reason as the operation unavailable reason before dispatch.
- Changed graph-backed Host Configuration Source inspection to fail closed
  when no host configuration source inspector is registered, and to project
  the same missing-inspector reason as the operation unavailable reason before
  dispatch.
- Changed graph-backed Docker Host inspection to fail closed when no Docker
  Host inspector is registered, and to project the same missing-inspector
  reason as the operation unavailable reason before dispatch.
- Changed graph-backed Container Host inspection to fail closed when no
  Container Host inspector is registered, and to project the same
  missing-inspector reason as the operation unavailable reason before dispatch.
...
- Changed graph-backed SQL Server lifecycle actions to fail closed when no SQL
  ...
  missing-handler reason as the operation unavailable reason before dispatch.
- Changed graph-backed Event Broker lifecycle actions to fail closed when no
...
- Changed graph-backed .NET app lifecycle actions to fail closed when no .NET
  ...
  missing-controller reason as the operation unavailable reason before
  dispatch.
- Changed graph-backed JavaScript app lifecycle actions to fail closed when no
...
- Added the MVP sample seam audit and updated the roadmap to treat the broad
  `CloudShell.Sample.Tests` run as green after the local Docker host pass with
  115 passing tests.
...
- Routed ASP.NET Core project lifecycle execution through the provider
  execution dispatcher with in-process start, stop, and restart handlers,
  keeping Application Topology's project resources on the MVP execution
  boundary.
...
- Routed Go app lifecycle execution through the provider execution dispatcher
  with in-process start, stop, and restart handlers, keeping the Go
  local-process sample on the same execution boundary.
- Routed Python app lifecycle execution through the provider execution
  dispatcher with in-process start, stop, and restart handlers, keeping the
  Python local-process sample on the same execution boundary.
...
- Routed Docker Host inspect execution through the provider execution
  dispatcher with an in-process runtime-observation handler.
- Routed Container Host inspect execution through the provider execution
  dispatcher with an in-process runtime-observation handler.
- Routed Storage inspect execution through the provider execution dispatcher
...
- Routed Configuration Store inspect execution through the provider execution
  dispatcher with an in-process runtime-observation handler.
- Routed Secrets Vault inspect execution through the provider execution
  dispatcher with an in-process runtime-observation handler.
...
- Added CoreShell notification producer contracts, notification/toast
...
- Added resource actions to failed single-resource template apply notifications
  when the target resource exists and suppressed resource targets for rejected
  single-resource applies when the target resource does not exist.
...
- Added the first Control Plane and remote client deployment-artifact API
  surface for resource-scoped direct host-store uploads, including store
...
- Added an experimental Go runtime SDK under `sdk/go/cloudshell` with the same
...
- CoreShell now owns section address-value resolution for full section IDs and
  child route values, and `CoreShell.Blazor` can project a section outlet by
...
- Resource Manager page and resource-detail links now use a CoreShell-backed
  ...
  href generation, replacing the old Composition UI link helper.
