...
public sealed class GoAppReferenceGraphValidator : IResourceDefinitionGraphValidator
{
        ...
        foreach (var reference in references)
        {
            if (reference.Relationship != ResourceReferenceRelationships.Reference)
            {
                diagnostics.Add(ResourceDefinitionDiagnostic.Error(
                    ...
                    $"Go app '{resource.EffectiveResourceId}' declares service reference '{reference.Value}' with relationship '{reference.Relationship}', expected '{ResourceReferenceRelationships.Reference}'.",
            ...
            if (!reference.TryGetResourceId(out var resourceId))
            {
                diagnostics.Add(ResourceDefinitionDiagnostic.Error(
                    ResourceDefinitionDiagnosticCodes.ResourceCapabilityReferenceInvalid,
                    $"Go app '{resource.EffectiveResourceId}' declares service reference '{reference.Value}' with addressing mode '{reference.AddressingMode}', expected '{ResourceReferenceAddressingModes.ResourceId}'.",
                    resource.EffectiveResourceId));
                continue;
            }
            ...
            if (target is null)
            {
                diagnostics.Add(ResourceDefinitionDiagnostic.Error(
                    ...
                    $"Go app '{resource.EffectiveResourceId}' references missing service resource '{resourceId}'.",
                    resource.EffectiveResourceId));
                continue;
            }
                ...
                target.Type.TypeId != reference.TypeId.Value)
            {
                diagnostics.Add(ResourceDefinitionDiagnostic.Error(
                    ...
                    $"Go app '{resource.EffectiveResourceId}' references resource '{target.EffectiveResourceId}' with type '{target.Type.TypeId}', expected '{reference.TypeId.Value}'.",
                    resource.EffectiveResourceId));
            }
        }
    }
