...
public interface IJavaAppRuntimeController
{
    JavaAppRuntimeStatus GetStatus(Resource resource);
...
public interface IJavaAppRuntimeOutputReader
{
    IReadOnlyList<JavaAppRuntimeOutputEntry> ReadOutput(
        string resourceId,
        int maxEntries = 200,
        DateTimeOffset? before = null);
}

public interface IJavaAppRuntimeMonitor
{
...
public interface IJavaAppRuntimeEnvironmentProvider
{
...
public sealed record JavaAppRuntimeOutputEntry(
    DateTimeOffset Timestamp,
    string Message,
    string Stream,
    string? Severity = null);

public enum JavaAppRuntimeStatus
{
    Unknown,
    Stopped,
    Running
...
public sealed class JavaAppProcessRuntimeController(
    IEnumerable<IJavaAppRuntimeEnvironmentProvider>? environmentProviders = null) :
    IJavaAppRuntimeController,
    IJavaAppRuntimeOutputReader,
    IJavaAppRuntimeMonitor,
    IDisposable,
    IAsyncDisposable
{
    ...
    private readonly ConcurrentDictionary<string, BoundedRuntimeOutputBuffer> _output = new(
        StringComparer.OrdinalIgnoreCase);
    private readonly JavaAppProcessCommandFactory _commands = new();
    private readonly IReadOnlyList<IJavaAppRuntimeEnvironmentProvider> _environmentProviders =
        (environmentProviders ?? []).ToArray();

    public JavaAppRuntimeStatus GetStatus(Resource resource)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        [
            ResourceDefinitionDiagnostic.Error(
                "application.javaApp.operationUnsupported",
                $"Java app runtime does not support operation '{operationId}'.",
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!Directory.Exists(fullProjectPath))
        {
            ...
            [
                ResourceDefinitionDiagnostic.Error(
                    "application.javaApp.projectDirectoryMissing",
                    $"Java app project directory '{fullProjectPath}' does not exist.",
                    resource.EffectiveResourceId)
            ];
        }
        ...
        try
        {
            if (!process.Start())
            {
                ...
                [
                    ResourceDefinitionDiagnostic.Error(
                        "application.javaApp.processStartFailed",
                        $"Java app process for '{resource.EffectiveResourceId}' did not start.",
                        resource.EffectiveResourceId)
                ];
            }
        }
        catch (Exception exception) when (exception is InvalidOperationException or System.ComponentModel.Win32Exception)
        {
            ...
            [
                ResourceDefinitionDiagnostic.Error(
                    "application.javaApp.processStartFailed",
                    $"Java app process for '{resource.EffectiveResourceId}' did not start. {exception.Message}",
                    resource.EffectiveResourceId)
            ];
        }
        ...
        string severity)
    {
        ...
        {
            output.Append(new JavaAppRuntimeOutputEntry(
    ...
    private sealed class BoundedRuntimeOutputBuffer(int capacity = 1_000)
    {
        ...
        private readonly Queue<JavaAppRuntimeOutputEntry> _entries = new();

        public void Append(JavaAppRuntimeOutputEntry entry)
        {
            lock (_lock)
            {
        ...
        public IReadOnlyList<JavaAppRuntimeOutputEntry> Read(
            int maxEntries,
...
public sealed class NoopJavaAppRuntimeController :
    IJavaAppRuntimeController,
    IJavaAppRuntimeOutputReader,
    IJavaAppRuntimeMonitor
{
    public JavaAppRuntimeStatus GetStatus(Resource resource) =>
        JavaAppRuntimeStatus.Unknown;
