...
public sealed class JavaScriptAppServiceDiscoveryEnvironmentResolver(
    ResourceGraphModel? graphModel = null) : IJavaScriptAppRuntimeEnvironmentProvider
{
        ...
        ResourceState resource)
    {
        ...
        if (resource.TypeId == AspNetCoreProjectResourceTypeProvider.ResourceTypeId)
        {
            return resource.ResourceAttributeValues.GetObject<NetworkingEndpointRequestValue[]>(
    ...
    private static IReadOnlyList<string> ResolveServiceDiscoveryNames(ResourceState resource)
    {
        var configured = resource.ResourceAttributeValues.TryGetValue(
            ResolveServiceDiscoveryAttribute(resource.TypeId),
            out var value) &&
    ...
    private static ResourceAttributeId ResolveServiceDiscoveryAttribute(
        ResourceTypeId typeId)
    {
        if (typeId == AspNetCoreProjectResourceTypeProvider.ResourceTypeId)
        {
            return AspNetCoreProjectResourceTypeProvider.Attributes.ServiceDiscoveryName;
        }

        if (typeId == JavaAppResourceTypeProvider.ResourceTypeId)
        {
            return JavaAppResourceTypeProvider.Attributes.ServiceDiscoveryName;
        }

        if (typeId == GoAppResourceTypeProvider.ResourceTypeId)
        {
            return GoAppResourceTypeProvider.Attributes.ServiceDiscoveryName;
        }

        if (typeId == PythonAppResourceTypeProvider.ResourceTypeId)
        {
            return PythonAppResourceTypeProvider.Attributes.ServiceDiscoveryName;
        }

        return JavaScriptAppResourceTypeProvider.Attributes.ServiceDiscoveryName;
    }
