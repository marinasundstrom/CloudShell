...
public sealed class GoAppResourceManagerStateProvider(
    ...
    IResourceModelResourceManagerStateProvider
{
    ...
    public ResourceManagerState? GetState(Resource resource)
    {
        ...
        if (resource.Type.TypeId != GoAppResourceTypeProvider.ResourceTypeId)
        {
            return null;
        }
        ...
        {
            GoAppRuntimeStatus.Running => ResourceManagerState.Running,
            GoAppRuntimeStatus.Stopped => ResourceManagerState.Stopped,
            _ => null
        };
    }
}

public sealed class GoAppResourceManagerEndpointProjectionProvider :
...
public sealed class GoAppResourceManagerObservabilityProvider :
    IResourceModelResourceManagerObservabilityProvider
{
        ...
        Resource resource)
    {
        ...
        return resource.Type.TypeId == GoAppResourceTypeProvider.ResourceTypeId
...
public sealed class GoAppResourceManagerMonitoringProvider(
    IGoAppRuntimeMonitor? runtimeMonitor = null) : IResourceMonitoringProvider
{
    private const string ProviderDisplayName = "Go app";
    private readonly IGoAppRuntimeMonitor _runtimeMonitor =
        runtimeMonitor ?? new NoopGoAppRuntimeController();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (snapshot is null)
        {
            return new ResourceMonitoringSnapshot(
                ...
                [],
                "Unavailable",
                "The Go app process could not be observed.");
        }
        ...
        return new ResourceMonitoringSnapshot(
            ...
            ResourceProcessMonitoringMetricSamples.Create(
                ...
                "Go app process"),
            "Available",
            "Go app process metrics.");
    }
}

public sealed class GoAppResourceManagerLogProvider(
    IGoAppRuntimeOutputReader? outputReader = null) : ILogProvider
{
    private readonly IGoAppRuntimeOutputReader _outputReader =
        outputReader ?? new NoopGoAppRuntimeController();

    public string Id => "resource-model.go-app.logs";

    public string DisplayName => "Go app logs";

    public IReadOnlyList<LogSource> GetLogSources() => [];

    public bool CanOpenLogSource(LogSource source) =>
        IsGoAppLogSource(source);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return ValueTask.FromResult<ILogSourceSession?>(
            IsGoAppLogSource(source) && source.ResourceId is not null
                ? new GoAppResourceManagerLogSourceSession(_outputReader, source)
                : null);
    }

    private static bool IsGoAppLogSource(LogSource source) =>
...
public sealed class GoAppResourceManagerLogSourceSession(
    IGoAppRuntimeOutputReader outputReader,
    LogSource source) : ILogSourceSession
{
    ...
    private static LogEntry ToLogEntry(GoAppRuntimeOutputEntry entry, LogFormat format) =>
