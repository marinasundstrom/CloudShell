...
public sealed class SampleSmokeTests
{
    ...
    public async Task ApplicationTopologyHost_DeclaresWorkloadThroughResourceModel()
    {
        ...
        Assert.Contains("Add variable", graphApiEnvironmentHtml);
        Assert.Contains("Environment variable source", graphApiEnvironmentHtml);
        Assert.Contains("CLOUDSHELL_TRACE_INGEST_ENDPOINT", graphApiEnvironmentHtml);
        Assert.Contains("CLOUDSHELL_METRIC_INGEST_ENDPOINT", graphApiEnvironmentHtml);
        Assert.Contains("OTEL_SERVICE_NAME", graphApiEnvironmentHtml);
