...
- Keep `docs/resource-identity-and-permissions.md` as the current-state
  feature documentation and `docs/proposals/core/identity-and-access.md` as
  the open-work tracker.
- Extend default resource identity-provider selection beyond the new provider
  ...
  contracts are now available on resources and the Control Plane API. Control
  Plane hosts can configure provider registrations through `ResourceIdentity`,
  and programmatic declarations can register provider definitions and select a
  default provider with `AddIdentityProvider(...)` and
  `UseDefaultIdentityProvider(...)`.
