...
| Hosting | [Hosting model](hosting-model.md), [Built-in service runtime](built-in-service-runtime.md), [Launchers](launchers-and-app-hosts.md), [Integration story](integration-story.md), [Control Plane execution future direction](future/control-plane-execution-model.md) | Current combined, split, and local-development hosting; Control Plane distributions, instances, endpoints, launcher responsibilities, and application-scoped versus daemon-supervised execution. |
...
| Configuration and secrets | [Configuration services](configuration-services.md), [Built-in service runtime](built-in-service-runtime.md), [Resource identity and permissions](resource-identity-and-permissions.md) | Configuration Store, Secrets Vault, process/container realization, reference flow, identity-backed access, and safe value handling. |
| Device identity and enrollment | [Device Registry](resources/device-registry.md), [Built-in service runtime](built-in-service-runtime.md), [Resource identity and permissions](resource-identity-and-permissions.md), [Event Broker](resources/event-broker.md) | Device Registry service resources, process/container realization, trust anchors, enrollment policy, device identity category, built-in identity-backed MVP provisioning, and the event-transport boundary for future device telemetry and check-ins. |
