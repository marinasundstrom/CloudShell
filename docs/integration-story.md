...
| --- | --- | --- | --- | --- |
| JavaScript/TypeScript | `samples/TypeScriptContainerApp` | TypeScript launcher source file using `Launchers/TypeScript/cloudshell` and `addJavaScriptApp(...).asContainerApp(...)`. | Reads Configuration Store and Secrets Vault through `sdk/typescript/configuration-client` from `/configuration` using injected endpoint and identity variables. | Launcher-authored container proof. |
