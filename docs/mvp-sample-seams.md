...
- Supported sample READMEs now call out the accepted local-development MVP
  bridges for SQL credentials, local DNS, Traefik ingress, host virtual
  networking, Keycloak identity, service-specific SDK endpoints, and deferred
  container-app deployment materialization.
...
- Graph-backed SQL database ensure-created now reports missing creation
  ...
  provider-execution diagnostic if invoked directly.
- Graph-backed executable application Start now reports missing
  runtime-controller readiness when the operation path is constructed without
  the built-in process runtime and returns a matching provider-execution
  diagnostic if invoked directly.
...
- Graph-backed Secrets Vault lifecycle actions now report missing
  runtime-controller readiness when the operation path is constructed without
  the built-in process runtime and return matching provider-execution
  diagnostics if invoked directly.
- Graph-backed Device Registry lifecycle actions now report missing
  runtime-controller readiness when the operation path is constructed without
  the built-in process runtime and return matching provider-execution
  diagnostics if invoked directly.
- Graph-backed .NET app lifecycle actions now report missing
  runtime-controller readiness when the operation path is constructed without
  the built-in process runtime and return matching provider-execution
  diagnostics if invoked directly.
- Graph-backed JavaScript app lifecycle actions now report missing
  runtime-controller readiness when the operation path is constructed without
  the built-in process runtime and return matching provider-execution
  diagnostics if invoked directly.
- Graph-backed Java app lifecycle actions now report missing
  ...
  the built-in process runtime and return matching provider-execution
  diagnostics if invoked directly.
- Graph-backed Go app lifecycle actions now report missing runtime-controller
  readiness when the operation path is constructed without the built-in
  process runtime and return matching provider-execution diagnostics if
  invoked directly.
- Graph-backed Python app lifecycle actions now report missing
  runtime-controller readiness when the operation path is constructed without
  the built-in process runtime and return matching provider-execution
  diagnostics if invoked directly.
