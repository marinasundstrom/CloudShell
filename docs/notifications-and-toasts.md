...
| Artifact upload, validation, and apply | Notification-backed | Upload/validate/apply has progress, failure diagnostics, revision targets, and resource targets. | Planned next high-value producer. |
| Deployment or replica reconciliation | Notification-backed when user-visible | Reconciliation should update an existing operation when possible and avoid noisy low-level progress. | Start-driven materialization and replica repair slices implemented; broader projection planned. |
