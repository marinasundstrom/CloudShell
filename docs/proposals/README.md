...
| 6 | [Resource monitoring](core/resource-monitoring.md) | In progress | MVP: Resource Manager UX polish; provider diagnostics | Tracks provider-observed resource metrics under the Management group. Current slices add provider-backed snapshots, Control Plane API/client support, a generated Monitoring tab, Docker container CPU/memory/network/block/process/restart/uptime metrics, and application process CPU/memory/thread/uptime metrics; next UX work includes a provider-owned container app Monitoring dashboard with app summaries and per-replica breakdowns. |
