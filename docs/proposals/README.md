...
| 9 | [DNS and name mapping](networking/dns-and-name-mapping-resource.md) | Proposed | MVP: Application exposure, discovery, and names; provider-backed DNS post-MVP | MVP should model logical names and UI relationships. Provider-backed DNS propagation and network-level service registries remain post-MVP unless needed by a concrete validation sample. |
