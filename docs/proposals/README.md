...
| 10 | [DNS and name mapping](networking/dns-and-name-mapping-resource.md) | In progress | MVP: Application exposure, discovery, and names; provider-backed DNS post-MVP | First logical DNS zone and name-mapping projection exists through programmatic declarations, and Resource Manager can create a DNS zone with one initial name mapping. Standalone name-mapping add/update UI, provider-backed DNS propagation, and network-level service registries remain open. |
