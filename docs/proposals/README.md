...
| 6 | [Resource monitoring](core/resource-monitoring.md) | In progress | MVP: Resource Manager UX polish; provider diagnostics | Tracks provider-observed resource metrics under the Management group. First slice adds provider-backed snapshots, Control Plane API/client support, generated Monitoring tab, and Docker container CPU/memory metrics. |
