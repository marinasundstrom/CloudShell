...
This table is the authoritative active proposal catalog. Keep it current
whenever proposal status, MVP relevance, fit, or remaining work changes.
Execution order is owned by the active queue in [Roadmap](../roadmap.md);
proposal index numbers below are for stable reference, not permission to pull
work ahead of the roadmap. Milestone scope remains authoritative in
[Roadmap](../roadmap.md).
...
| Index | Proposal | Status | Fit likelihood | Action |
| --- | --- | --- | --- | --- |
...
| 15 | [Deployment artifacts](core/deployment-artifacts.md) | Proposed | High; required for create/edit UI that works when the Control Plane and workload host are remote. | Wire the first application resource provider and Resource Manager create/edit section to the landed resource-scoped artifact upload API before broad app create/edit UX. |
...
| 23 | [Cross-language local development](core/cross-language-local-development.md) | In progress | High; required to keep CloudShell ecosystem-neutral. | Harden the installed CLI, project-local YAML, and same-version foreground development-host MVP, then align launcher/profile and SDK work with that boundary. |
...
| [CloudShell agents and clustering](../future/agents-and-clustering.md) | High long-term fit for scaling CloudShell-managed services and host-local execution across machines, regions, and data-center-like boundaries. | Keep MVP relevance limited to agent/reconciliation-shaped execution boundaries. Defer remote agents, placement, regions, clustering, and multi-host replicas until local MVP and first on-premise hosting needs justify a slice. |
...
Use [Roadmap](../roadmap.md) for the active MVP queue and concrete execution
plan. Proposal work should be pulled only when it improves that queue:
supported sample confidence, the app-centric Resource Manager loop,
container-app runtime coherence, routing/name/readiness diagnostics, or
intentional documentation of accepted MVP bridges.

The active MVP queue maps to proposal areas this way:

1. **Classify exposed seams in supported samples** through
   ...
   and the relevant resource/provider proposal for the exposed seam.
2. **Tie off Application Topology first** through
   [Container applications](containers/container-applications.md),
   [Storage and volume mappings](storage/volume-mappings.md),
   [Secrets management](services/secrets-management.md),
   [Identity and access](core/identity-and-access.md),
   [DNS and name mapping](networking/dns-and-name-mapping-resource.md), and
   [Load balancer resource](networking/load-balancer-resource.md).
3. **Make exposed container-app runtime behavior coherent** through
   [Container applications](containers/container-applications.md),
   [Orchestrator deployments and environment revisions](deployment/deployments-and-revisions.md),
   [Provider-created and runtime-managed resources](core/provider-created-and-runtime-managed-resources.md),
   and [Container host abstraction](containers/container-host-abstraction.md).
4. **Close routing, name, and readiness rough edges** through
   ...
   [Container host abstraction](containers/container-host-abstraction.md), and
   resource-specific diagnostics proposals as needed.
5. **Smooth Resource Manager paths and document bridges** through
   [Platform foundations](core/platform-foundations.md), the relevant feature
   docs, and proposal cleanup where landed behavior has already moved into a
   specification.

The ordered backlog remains valid but should not displace the active queue:

- Configuration, secrets, and identity polish.
- Persisted-state handoff and launcher/profile data-root clarity.
- Storage and volume materialization diagnostics beyond the current local
  bridge.
- Identity validation beyond the built-in provider, keeping Keycloak smoke
  coverage green.
- Lifecycle traceability, audit, logging, monitoring, recovery, and degradation
  slices that directly improve app-centric diagnosis.
- Host and runtime foundation work, including remote Docker hosts, only when
  it validates the shared host abstraction or unblocks supported samples.
- Runtime-managed resource enrichment where container apps need observed IDs,
  health, placement, or materialization diagnostics.
- CoreShell, UI composition, notifications, deployment artifacts,
  cross-language hardening, and intent-first authoring only where the roadmap
  active queue calls for them.

Agent/reconciliation work is a guardrail, not a separate active proposal:
shape host-local execution changes so they can later become typed
agent-side assignments with leases, desired/observed state, idempotency, and
diagnostics. Remote agents, placement, regions, clustering, and multi-host
replica scheduling remain future-direction work until the roadmap promotes a
near-term slice.
