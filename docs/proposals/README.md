...
| 10 | [Storage and volume mappings](storage/volume-mappings.md) | In progress | MVP: Storage and volume mappings | Initial `cloudshell.volume`, `AddVolume(...)`, `ResourceVolumeMount`, mount permissions, container app selector/workload projection, dedicated Storage tab, and direct volume create/configuration/overview UI are in place. Next work is runtime materialization, provider-defined storage resources, relationship visibility, and usage monitoring. |
