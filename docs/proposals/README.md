...
| 10 | [Storage and volume mappings](storage/volume-mappings.md) | In progress | MVP: Storage and volume mappings | Initial `cloudshell.volume`, `AddVolume(...)`, `ResourceVolumeMount`, and container app declaration/workload projection are in place. Next work is runtime materialization, provider-backed volume resources, UI management, and usage monitoring. |
