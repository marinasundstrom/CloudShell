...
| 10 | [Storage and volume mappings](storage/volume-mappings.md) | In progress | MVP: Storage and volume mappings | Initial `cloudshell.volume`, `AddVolume(...)`, `ResourceVolumeMount`, mount permissions, container app selector/workload projection, and dedicated Storage tab are in place. Next work is runtime materialization, provider-backed volume resources, relationship visibility, and usage monitoring. |
