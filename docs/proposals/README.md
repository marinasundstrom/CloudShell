...
| 17 | [Shell composition](core/shell-composition.md) | Proposed | Post-MVP extensible shell platform | Tracks the future CloudShell UI direction: a layout/content engine with separate navigation and addressable content hierarchies, content-ID link resolution, Razor-owned routing, slots, section containers, standard settings, notifications, and Resource Manager alignment with generic shell primitives. |
