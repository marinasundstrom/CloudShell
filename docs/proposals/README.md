...
| 22 | [Resource graph and runtime separation](core/resource-definitions-and-capability-providers.md) | POC in progress; switch-readiness integration | Later authoring, persistence, and provider model foundation | Initial isolated `CloudShell.ResourceDefinitions` project proves the graph/configuration contract, definition envelope, inheritance resolver, diagnostics, and attached provider contracts. Graph-backed provider ports now cover selected sample workflows through temporary runtime seams; the proposal defines an integration-first switch gate where runnable main workflows and documented non-parity are enough to start controlled switch-over before final seam cleanup. |
