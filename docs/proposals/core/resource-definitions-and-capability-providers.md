...
| ContainerHost | The sample host now declares graph-backed storage, volume, and SQL Server resources. Service-level coverage verifies Resource Manager projection, storage/volume attributes, typed storage dependency, and SQL Server volume-consumer capability. The sample now defaults to graph-only mode, omitting the old application-provider storage, volume, and SQL Server records plus the old application provider registration while keeping graph SQL Docker lifecycle coverage active through the sample-local bridge. | Use this sample to keep validating storage-backed volume semantics before runtime storage materialization moves behind graph-backed provider seams. |
