...
| ApplicationTopology | The sample host now declares side-by-side graph-backed storage, SQL Server/database, configuration, secrets, host-configuration source, API, and frontend ASP.NET Core resources across provider boundaries, and it now defaults to a graph-only declaration mode that omits the old application/configuration/secrets/storage/volume/SQL workload resource records and skips the old application, configuration, and secrets provider registrations. Smoke coverage verifies graph-only resource projection, absence of the old workload declarations, local DNS retargeting to the graph frontend, graph host-configuration source projection without exposed values, the full graph storage -> volume -> SQL Server -> database chain, and graph-backed Configuration Store, Secrets Vault, API, and frontend startup. Side-by-side smoke coverage explicitly opts back into the old provider path and still verifies generated Resource Manager UI pages, SQL endpoint mapping, settings/secrets endpoints, typed startup dependencies, graph frontend-to-API service discovery through `project.serviceDiscoveryName` and `project.references`, health/liveness declarations, and explicit separation from `DependsOn`. The graph Configuration Store and Secrets Vault now use separate configurable service endpoints, provider-owned runtime option registration, seeded provider-owned runtime data, Resource Manager-declared graph API identity grants, backing-service startup, authenticated entry/secret reads through the new provider runtime controllers, and graph API consumption through the CloudShell Configuration Store and Secrets Vault client integrations derived from `project.references`. The graph ASP.NET Core resources now use runtime-ready absolute project paths, separate configurable graph endpoints, and runtime smoke coverage for graph API settings, frontend-to-API discovery through the intentional failure route, graph API `/database`, and the Docker-backed graph frontend `/upstream` path. The API project can target either the old or graph SQL Server service-discovery name through configuration, and the graph API declares the graph SQL target, built-in identity, SQL read/write grant, sample-local graph SQL lifecycle adapter with cached status projection, graph SQL deterministic adapter coverage, graph SQL start/restart/stop smoke coverage, SQL container create/recreate/remove assertions, and sample-local graph SQL credential endpoint for `/database`. Graph SQL lifecycle delegation now sits behind `IApplicationTopologyGraphSqlServerRuntimeBridge`; side-by-side mode uses the old Resource Manager SQL bridge, while graph-only mode uses a sample-local Docker-backed bridge. The graph-only SQL declaration now uses explicit `cloudshell.storage` and storage-backed `cloudshell.volume` resources, and the Docker bridge resolves that graph shape into a SQL data bind mount before start. Docker smoke coverage verifies graph-only SQL startup, graph database creation, graph API `/database`, graph frontend `/upstream`, SQL data directory materialization, and graph SQL cleanup without declaring the old SQL resource. The graph-only SQL bridge now contributes a Resource Manager orchestration descriptor with `ControlPlaneScoped` lifetime, and smoke coverage verifies graceful host shutdown removes the graph SQL Docker container through the host-scoped shutdown service. | Use this sample to keep validating multi-provider dependencies, host-configuration source metadata, SQL child ownership, volume materialization, graph SQL credential flow, generated Resource Manager UI parity, and application-level discovery before replacing the old application-provider path. Graph SQL credential brokering, graph-backed environment editing projection, provider-owned SQL runtime implementation, generalized graph-backed workload descriptor projection, and removal of old provider package bridge dependencies remain provider work. |
| ContainerAppDeployment | The sample host now declares only Resource Definitions-backed Docker host, registry container, and container-app resources with stable IDs (`docker:sample`, `docker.container:sample-registry`, and `application.container-app:sample-api`) through provider-owned resource graph builders. The old Docker/application provider records, old provider registrations, side-by-side comparison path, and `GraphOnly` setting have been removed for this sample. Smoke coverage verifies Resource Manager projection, registry attributes, typed dependencies, deployment image updates, replica updates, and generated container-app UI panels against the new resource IDs. Docker container lifecycle operations have a provider-owned runtime handler seam with Resource Manager state projection and status-aware action availability. The sample carries an opt-in registry materializer for `docker.container:sample-registry` behind `ContainerAppDeployment:EnableDockerRuntime`, disabled by default so normal projection does not shell out to Docker. The materializer uses a sample-local Docker command runner with deterministic status and lifecycle command coverage, bounded cached status projection, and Docker smoke coverage that verifies registry start, `/v2/` availability, stop, container cleanup, and graceful host shutdown cleanup without old provider records. The Resource Model bridge can satisfy the existing deployment image-update API and replica update API for container apps by applying `container.image` and/or `container.replicas` changes into the graph before executing temporary image-update or replica-update operation seams. Those seams are POC adapter hooks for accepted graph changes because the current API still models image deployment and scaling as Control Plane workflows; they do not decide whether image upload, deployment, or non-lifecycle scale workflows should later become explicit resource operations. | Port runtime deployment only when the graph-to-Resource Manager runtime apply boundary can be tested against container-host seams without reintroducing the old application-provider aggregate. Keep registry materialization opt-in so normal sample projection tests do not slow down or race on Docker CLI availability. |
| ReplicatedContainerHealth | The sample host now declares side-by-side graph-backed Docker host and replicated container-app resources. Smoke coverage verifies Resource Manager projection, replica-count attributes, typed graph dependencies, container endpoint requests, endpoint/network mapping projection, health/liveness capability declarations, and resource group template export of graph resources as `resource-definition.v1` payloads. Container-app start/stop/restart/image-update/replica-update graph operations now delegate through a provider-owned runtime handler seam, and the sample wires that seam to an explicit sample-local bridge contract. In side-by-side comparison mode the current bridge starts, stops, and restarts the existing `application:api` runtime app from the graph `application.container-app:graph-api` lifecycle actions, projects graph container-app state from the runtime app through the Resource Manager bridge state provider, and applies graph `container.image` or `container.replicas` changes into the existing runtime app configuration through the graph operations. Focused tests cover the replaceable bridge boundary, and Docker smoke coverage verifies the graph start/restart actions publish the replicated API health endpoint, graph restart recreates the revision-scoped runtime containers, graph stop projects stopped state, and graph stop removes the revision-scoped runtime containers created by graph start. The sample now defaults to graph-only declaration/runtime mode, skips the old application and Docker provider registrations, omits `application:api` plus `docker:sample`, and wires the graph container app to a sample-local Docker bridge that publishes the API image, starts/removes graph-owned replica containers, restarts them when graph image or replica attributes are applied, removes a bounded range of graph-owned replica containers so scale-down does not leave stale higher-ordinal replicas running, projects basic running/stopped state through bounded, cached Docker inspection, publishes probe-only ports for each graph-owned replica, contributes provider-projected replica container log sources for the current graph replica count, projects hidden runtime-managed replica resources through the existing flat `IResourceProvider` adapter for Control Plane runtime-scope health aggregation, gives those hidden runtime resources Resource Manager logs/traces/metrics observability metadata, and assigns each created replica container the projected runtime replica resource ID plus telemetry scope environment. Graph-only Docker smoke coverage now verifies image update, replica update, stale replica removal, app-owned ingress cleanup, graph-declared HTTP health/liveness refresh, runtime-scope health aggregation, log source discovery, Docker log reading, running-container observability environment, Resource Manager observability projection for hidden runtime replicas, and live trace/metric ingestion under the projected runtime replica resource ID without the old provider records. The provider README records the temporary switch seams: the side-by-side Resource Manager bridge, the graph-only Docker bridge, graph-only runtime resource provider, graph-only log provider, its command runner, bounded status probing and replica cleanup, runtime replica observability environment, Docker-reachable Control Plane ingest endpoint wiring, generated telemetry-tab parity gap for hidden runtime resources, container-app startup state projection gap, and the sample-local graph image update endpoint. A graph-only Docker smoke investigation confirmed that opening a hidden runtime replica's generated Traces details route can time out even though telemetry is ingested and queryable through the Control Plane API. The current runtime-boundary decision is to keep graph-only Docker bridge/log/runtime-resource behavior sample-local instead of bridging back into the old application-provider aggregate or promoting it into a shared provider runtime before another port proves the same seam. | Complete metrics/traces UI parity and richer Resource Manager state projection without reintroducing the old application-provider aggregate. In particular, generated details telemetry tabs for hidden runtime replica resources need a runtime-resource-specific details path so trace/metric rendering is not blocked by generic generated details work. The graph-only Docker bridge currently projects only running/stopped/unknown states; after the switch, the durable runtime should represent container startup separately so newly-started replicas can show "starting" while health converges instead of briefly appearing degraded/unhealthy, and should decide whether an initial no-replica projection should be unknown or stopped. After the switch succeeds, sweep the temporary seams by moving/porting required runtime behavior into the new provider/runtime structure and removing old-provider bridge paths. Use this sample later to decide which health/telemetry declarations belong in graph configuration and which remain operational projections from Resource Manager runtime state. |
| HostVirtualNetwork and LoadBalancer | Network, virtual network, host networking, DNS zone, name mapping, service, and load-balancer providers have definition/projection coverage. HostVirtualNetwork now carries only Resource Definitions-backed local host networking, ASP.NET Core target API, and virtual-network resources in the sample host; the old application-provider registration plus direct Resource Manager comparison records and comparison toggle have been removed while the endpoint-mapping runtime bridge remains sample-local. LoadBalancer now carries only Resource Definitions-backed Docker host, container-app targets, load-balancer, DNS zone, and name-mapping resources with stable sample identities such as `docker:sample-host`, `application.container-app:api`, `load-balancer:public`, and `dns:cloudshell-local`. The old Docker/application/load-balancer/DNS resource records, provider registrations, graph-prefixed sample names, and comparison toggle have been removed while route generation and DNS publishing remain active through sample-local runtime bridges. Smoke coverage verifies Resource Manager projection, typed dependencies, load-balancer count/operation shape, virtual-network endpoint mapping runtime handoff, HostVirtualNetwork switch readiness without old records, load-balancer dynamic configuration, DNS publishing to a temporary hosts file, name-mapping projection without old application records, and LoadBalancer switch readiness through sample-local runtime bridges. | Continue validating endpoint mapping operations, route payloads, and provider-owned network/load-balancer reconciliation as the graph model starts carrying richer route and backend configuration. |
