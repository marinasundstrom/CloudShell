...
| HostVirtualNetwork and LoadBalancer | Network, virtual network, host networking, DNS zone, name mapping, service, and load-balancer graph providers have definition/projection coverage. HostVirtualNetwork now also carries side-by-side graph-backed local host networking, ASP.NET Core target API, and virtual-network resources in the sample host, with smoke coverage for Resource Manager projection and typed graph dependencies. | Use these samples to validate endpoint mapping operations and provider-owned network reconciliation after application and settings dependencies are stable. LoadBalancer should be the next networking sample when route and backend projection need a broader test. |
