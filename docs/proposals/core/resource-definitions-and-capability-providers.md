...
| ContainerHost | The sample host now declares graph-backed storage, volume, and SQL Server resources. Service-level coverage verifies Resource Manager projection, storage/volume attributes, typed storage dependency, and SQL Server volume-consumer capability. The sample now defaults to graph-only mode, omitting the old application-provider storage, volume, and SQL Server records plus the old application provider registration while keeping graph SQL Docker lifecycle coverage active through the sample-local bridge. The generic sample-host readiness smoke test now removes the graph SQL container after launch so the detached sample-local runtime artifact does not leak into later Docker tests. | Use this sample to keep validating storage-backed volume semantics before runtime storage materialization moves behind graph-backed provider seams. The graph SQL runtime bridge still needs to participate in the Resource Manager host-scoped workload/shutdown path so programmatically declared graph resources match the old provider model instead of relying on test cleanup. |
