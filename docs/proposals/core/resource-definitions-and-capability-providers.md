...
| ThirdPartyIdentity | The sample host now declares a side-by-side graph-backed identity provisioning resource for Keycloak through the provider-owned identity builder. It also declares graph-backed Configuration Store and ASP.NET Core project resources for the protected workload shape, including endpoint projection, typed startup dependency, provider-owned configuration reference, entry-count summary projection, and Resource Manager-declared graph API identity bound to the graph Keycloak provider definition. Non-Docker smoke coverage verifies Resource Manager projection and setup operation shape, focused adapter coverage verifies that graph setup goes through an explicit sample-local bridge and that the current bridge delegates to the attached Resource Manager identity provider setup service, and the Docker-backed sample path executes graph setup through the Keycloak runtime integration with returned provider diagnostics before the old Resource Manager path provisions the workload identity and protected configuration access. The graph Configuration Store runtime can now be configured with external Keycloak service-bearer validation, and Docker smoke coverage starts the graph Configuration Store plus graph ASP.NET Core API and verifies the graph API reads protected graph configuration with a Keycloak-issued resource identity token from the graph identity provider. The sample now defaults to graph-only declaration mode that skips the old application and configuration provider registrations plus the old `identity-provisioning:keycloak`, `configuration:third-party-identity`, and `application:keycloak-provisioned-api` resource records while keeping Resource Manager-owned identity provider declarations and graph runtime integrations. Docker smoke coverage now runs the Keycloak-protected graph Configuration Store and graph ASP.NET Core API through that graph-only path, executes graph identity-provider setup, verifies protected graph configuration reads without old provider records, and verifies generated Keycloak compose cleanup after the graph-only path completes. The graph ASP.NET Core process runtime has a provider-local environment seam, and this sample wires a sample-local adapter that derives graph API identity credential variables from the Resource Manager identity declaration and existing Keycloak credential provider without depending on the old application provider in graph-only mode. | Decide whether individual resource identity declarations should remain Resource Manager-owned for the POC or gain first-class graph declarations after more provider ports need that shape. Keep the Keycloak credential adapter sample-local until another provider proves a shared resource-identity runtime environment seam. |
...
| HostVirtualNetwork and LoadBalancer | Network, virtual network, host networking, DNS zone, name mapping, service, and load-balancer graph providers have definition/projection coverage. HostVirtualNetwork now carries only Resource Definitions-backed local host networking, ASP.NET Core target API, and virtual-network resources in the sample host; the old application-provider registration plus direct Resource Manager comparison records and comparison toggle have been removed while the endpoint-mapping runtime bridge remains sample-local. LoadBalancer carries graph-backed Docker host, container-app targets, load-balancer, DNS zone, and name-mapping resources, and now defaults to graph-only mode that omits the old Docker/application/load-balancer/DNS resource records and provider registrations while keeping graph route generation and graph DNS publishing active through sample-local runtime bridges. Smoke coverage verifies Resource Manager projection, typed graph dependencies, load-balancer count/operation shape, virtual-network endpoint mapping runtime handoff, HostVirtualNetwork switch readiness without old records, graph load-balancer dynamic configuration, graph DNS publishing to a temporary hosts file, graph name-mapping projection without old DNS/name-mapping records, and graph-only LoadBalancer switch readiness through sample-local runtime bridges. | Continue validating endpoint mapping operations, route payloads, and provider-owned network/load-balancer reconciliation as the graph model starts carrying richer route and backend configuration. |
...
or provider conventions and should usually be left implicit. Builder methods
such as `WithResourceId(...)` exist for explicit platform ids, migration
cases, and current bridge seams where the old and new models must share an
identifier during provider porting. Display names remain convenience labels,
not identity.
...
| LoadBalancer | Graph load-balancer, DNS/name-mapping projection without old DNS records, and Traefik-backed routing paths. |
