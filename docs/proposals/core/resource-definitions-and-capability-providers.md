...
dependencies for the POC. This lets the POC validate target resolution without
introducing a separate reference collection before provider requirements prove
one is needed. Port, endpoint, and target collections remain future typed
payloads or capability members instead of normal count attributes.
