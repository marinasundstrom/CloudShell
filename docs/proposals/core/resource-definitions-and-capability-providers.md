...
| SplitHosting | The split Control Plane host now exposes only the graph-backed network resource through the Resource Manager bridge. The old persisted `network:split-sample` comparison record and `SplitHosting:GraphOnly` toggle have been removed, and smoke coverage verifies the separate UI host can render the graph-backed resource through the remote Control Plane client while the Control Plane API returns only the graph network. The supported sample-host launch matrix also authenticates through the sample client-credentials flow and verifies the protected split Control Plane resources API projects graph resources. | Use this sample to keep validating that graph-backed Resource Manager projections work across split hosting before changing public API/client contracts. |
...
| ContainerHost | The sample host now declares only graph-backed storage, volume, and SQL Server resources. Service-level coverage verifies Resource Manager projection, storage/volume attributes, typed storage dependency, and SQL Server volume-consumer capability. The old application-provider storage, volume, and SQL Server records, old application provider registration, and `ContainerHost:GraphOnly` comparison toggle have been removed while graph SQL Docker lifecycle coverage remains active through the sample-local bridge. The graph SQL bridge now contributes a Resource Manager orchestration descriptor with `ControlPlaneScoped` lifetime, and smoke coverage verifies graceful host shutdown removes the graph SQL Docker container through the host-scoped shutdown service. The generic sample-host readiness smoke test still removes the graph SQL container after launch because that harness kills sample processes instead of running hosted shutdown. | Use this sample to keep validating storage-backed volume semantics before runtime storage materialization moves behind graph-backed provider seams. Generalized graph-backed workload descriptor projection, durable provider-backed storage materialization, and default/preferred container-host placement remain provider work. |
...
| HostVirtualNetwork and LoadBalancer | Network, virtual network, host networking, DNS zone, name mapping, service, and load-balancer graph providers have definition/projection coverage. HostVirtualNetwork now carries only graph-backed local host networking, ASP.NET Core target API, and virtual-network resources in the sample host; the old application-provider registration plus old local-host networking, target API, virtual-network records, and `HostVirtualNetwork:GraphOnly` comparison toggle have been removed while the graph endpoint-mapping runtime bridge remains sample-local. LoadBalancer carries graph-backed Docker host, container-app targets, load-balancer, DNS zone, and name-mapping resources, and now defaults to graph-only mode that omits the old Docker/application/load-balancer/DNS resource records and provider registrations while keeping graph route generation and graph DNS publishing active through sample-local runtime bridges. Smoke coverage verifies Resource Manager projection, typed graph dependencies, load-balancer count/operation shape, graph virtual-network endpoint mapping runtime handoff, HostVirtualNetwork switch readiness without old records, graph load-balancer dynamic configuration, graph DNS publishing to a temporary hosts file, graph name-mapping projection without old DNS/name-mapping records, and graph-only LoadBalancer switch readiness through sample-local runtime bridges. | Continue validating endpoint mapping operations, route payloads, and provider-owned network/load-balancer reconciliation as the graph model starts carrying richer route and backend configuration. |
...
or Docker runtime design. Docker smoke coverage now exercises the graph SQL
...
seam. The bridge is intentionally sample-local: it projects graph resources
through the Resource Manager endpoint projection providers, adapts the graph
local-host networking resource identity to the existing local-host provisioner
contract, and delegates materialization to the Resource Manager endpoint
mapping provisioner. This demonstrates the intended glue boundary without
moving host proxy runtime behavior into the graph provider itself. Smoke
coverage starts the graph ASP.NET Core target, invokes the graph virtual
network reconcile action through the Control Plane API, and verifies the graph
public ingress forwards to the target health endpoint.
...
| `HostVirtualNetworkSample_ReconcilesGraphEndpointMappingThroughRuntimeBridge` | Proves graph endpoint mapping reconciliation through the sample runtime bridge. |
