...
| SettingsAndSecrets | Configuration Store, Secrets Vault, identity provisioning, and ASP.NET Core project graph definitions can be applied together; Configuration Store and Secrets Vault now project Resource Manager endpoints and `/healthz` declarations through provider-bridge coverage, and the sample host carries side-by-side graph-backed settings/secrets resources with endpoint, count-summary, and inspect-action projection/execution coverage. | Decide how graph-backed configuration/secrets resources hand off entries, secrets, grants, and protected backing services to Control Plane-owned runtime handlers without storing entries or secret values in graph state. |
...
| Pull health and liveness declarations into the graph model | In progress | The graph model now has a `health.checks` capability payload for HTTP health/liveness declarations, and Resource Manager bridge projections map those declarations to `ResourceHealthCheck` plus the derived `liveness` capability. The ProjectReference graph-backed ASP.NET Core project now declares `/health` and `/alive` through this payload and verifies that Control Plane health refresh can evaluate both probes through the projected endpoint mapping. Configuration Store and Secrets Vault graph providers now declare `/healthz` health checks on their projected service endpoints, while runtime health materialization waits for the Control Plane-owned backing service handoff. Polling, observed snapshots, degradation policy, and recovery decisions remain Control Plane concerns. |
...
| ASP.NET Core project (`application.aspnet-core-project`) | Modeled as a narrow reference provider | Project path, arguments, hot reload, launch-settings attributes, provider-owned typed endpoint request and environment-variable attributes, shared volume-consumer capability, provider-declared default console log source, start/stop/restart operations with state-sensitive runtime availability and a provider-owned process runtime controller that can derive `--urls` from endpoint requests and apply non-secret environment variables, typed wrapper, Resource Manager bridge projection/execution, optional endpoint projection through the bridge, log, metric, trace, health, Resource Manager details-tab coverage, ResourceDefinition update/apply smoke coverage with restart-required diagnostics from ProjectReference, and Control Plane start/stop smoke coverage from ProjectReference through graph-declared environment variables and provider-local runtime seams, ApplicationTopology, SettingsAndSecrets, and ProjectReference sample-inspired graph coverage | Launch settings parsing, richer process diagnostics, reusable environment-variable conventions if later providers prove the need, container build behavior, UI registration/update flow |
...
| Configuration store (`configuration.store`) | Modeled as a narrow reference provider | Configuration class/type defaults, endpoint and read-only entry-count summary attributes, `/healthz` health declaration, inspect operation with an injected provider-owned inspector seam, typed wrapper, Resource Manager bridge projection/execution, optional Resource Manager endpoint projection through a companion provider-bridge package, and SettingsAndSecrets smoke coverage for endpoint plus inspect-action projection/execution | Real configuration service runtime integration, entry storage, authorization, logs, templates, and UI registration/update flow |
...
| Secrets Vault (`secrets.vault`) | Modeled as a narrow reference provider | Secrets Vault class/type defaults, endpoint and read-only secret-count summary attributes, `/healthz` health declaration, inspect operation with an injected provider-owned inspector seam, typed wrapper, Resource Manager bridge projection/execution without storing secret values, optional Resource Manager endpoint projection through a companion provider-bridge package, and SettingsAndSecrets smoke coverage for endpoint plus inspect-action projection/execution | Real Secrets Vault runtime integration, secret storage, authorization, logs, templates, and UI registration/update flow |
| Identity provisioning (`cloudshell.identity-provisioning`) | Modeled as a narrow reference provider | Infrastructure class/type defaults, provider/provider-kind attributes, passive identity-provisioning capability marker, setup operation with an injected provider-owned setup handler seam, typed wrapper, apply planning, and Resource Manager bridge projection/execution | Real identity provider setup, directory/client materialization, credential issuance, grant reconciliation, authorization, diagnostics, and UI registration/update flow |
