...
| HostVirtualNetwork and LoadBalancer | Network, virtual network, host networking, DNS zone, name mapping, service, and load-balancer graph providers have definition/projection coverage. HostVirtualNetwork carries graph-backed local host networking, ASP.NET Core target API, and virtual-network resources in the sample host, and now defaults to graph-only mode that omits the old application-provider registration plus old local-host networking, target API, and virtual-network records while keeping the graph endpoint-mapping runtime bridge active. LoadBalancer carries graph-backed Docker host, container-app targets, load-balancer, DNS zone, and name-mapping resources, and now defaults to graph-only mode that omits the old Docker/application/load-balancer/DNS resource records and provider registrations while keeping graph route generation and graph DNS publishing active through sample-local runtime bridges. Smoke coverage verifies Resource Manager projection, typed graph dependencies, load-balancer count/operation shape, graph virtual-network endpoint mapping runtime handoff, graph-only HostVirtualNetwork switch readiness, graph load-balancer dynamic configuration, graph DNS publishing to a temporary hosts file, graph name-mapping projection without old DNS/name-mapping records, and graph-only LoadBalancer switch readiness through sample-local runtime bridges. | Continue validating endpoint mapping operations, route payloads, and provider-owned network/load-balancer reconciliation as the graph model starts carrying richer route and backend configuration. |
