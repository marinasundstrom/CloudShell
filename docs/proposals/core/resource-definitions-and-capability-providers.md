...
| HostVirtualNetwork and LoadBalancer | Network, virtual network, host networking, DNS zone, name mapping, service, and load-balancer graph providers have definition/projection coverage. HostVirtualNetwork carries side-by-side graph-backed local host networking, ASP.NET Core target API, and virtual-network resources in the sample host. LoadBalancer carries side-by-side graph-backed Docker host, container-app targets, load-balancer, DNS zone, and name-mapping resources. Smoke coverage verifies Resource Manager projection, typed graph dependencies, load-balancer count/operation shape, graph virtual-network endpoint mapping runtime handoff, graph load-balancer dynamic configuration, and graph DNS publishing to a temporary hosts file through sample-local runtime bridges. | Continue validating endpoint mapping operations, route payloads, and provider-owned network/load-balancer reconciliation as the graph model starts carrying richer route and backend configuration. |
