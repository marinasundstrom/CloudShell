...
| --- | --- | --- |
| ProjectReference | Graph-backed ASP.NET Core API and frontend resources run side-by-side with old application-provider resources and now prove graph-to-graph project calls through service-discovery variables derived from provider-owned graph references, provider-owned service names, endpoint request attributes, start, stop, logs, traces, metrics, health, endpoint projection, state projection, and ResourceDefinition apply diagnostics. ASP.NET Core state, endpoint, observability, and process-output log projection now come from the reference-provider bridge package. Provider-owned service references now validate missing targets and expected resource type mismatches without using `DependsOn`. The sample also has an opt-in graph-only mode that omits the old application-provider project records and registration while keeping graph ASP.NET Core startup and graph-to-graph service discovery coverage. | Decide whether ASP.NET Core graph service discovery should remain provider-local for the POC or become a reusable graph/runtime projection after more providers need it; updated side-by-side smoke coverage still keeps old `application:project-reference-api` dependency links as the comparison baseline. |
| SplitHosting | The split Control Plane host now exposes a graph-backed network resource through the Resource Manager bridge. It has an opt-in graph-only mode that omits the old persisted network record, and smoke coverage verifies the separate UI host can render the graph-backed resource through the remote Control Plane client while the Control Plane API returns only the graph network. | Use this sample to keep validating that graph-backed Resource Manager projections work across split hosting before changing public API/client contracts. |
...
| ContainerHost | The sample host now declares graph-backed storage, volume, and SQL Server resources. Service-level coverage verifies Resource Manager projection, storage/volume attributes, typed storage dependency, and SQL Server volume-consumer capability. The sample also has an opt-in graph-only mode that omits the old application-provider storage, volume, and SQL Server records plus the old application provider registration while keeping graph SQL Docker lifecycle coverage active through the sample-local bridge. | Use this sample to keep validating storage-backed volume semantics before runtime storage materialization moves behind graph-backed provider seams. |
