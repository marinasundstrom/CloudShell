...
| SplitHosting | The split Control Plane host now exposes a graph-backed network resource through the Resource Manager bridge. It defaults to graph-only mode, omitting the old persisted network record, and smoke coverage verifies the separate UI host can render the graph-backed resource through the remote Control Plane client while the Control Plane API returns only the graph network. | Use this sample to keep validating that graph-backed Resource Manager projections work across split hosting before changing public API/client contracts. |
