...
ASP.NET Core process environment derivation is now a provider-local runtime
extension point. The graph provider contributes service-discovery variables
from graph references by default, and samples or Control Plane integrations
can contribute additional runtime variables, such as resource identity
credentials, without making identity part of the graph-native ASP.NET Core
resource type.
...
owned by the ASP.NET Core built-in provider.
...
The later end-to-end proof is to run the CloudShell UI against graph-backed
providers and verify that Resource Manager can inspect and operate the same
workloads through the normal user experience. Until the provider switch-over is
...
| Graph-safe application type metadata | Graph-only samples can register application resource type metadata without registering old application providers. ASP.NET Core project, SQL Server, executable, and container app graph resources get stable display names, icons, endpoint descriptors, and probe defaults for generated Resource Manager views and Add Resource catalog placeholders. | Keep this metadata-only until a graph-backed declaration/deployment authoring flow replaces old register/update pages. |
| Graph-safe non-application type metadata | The Resource Manager UI support project now also registers metadata placeholders for SQL database children, Configuration Store, Secrets Vault, identity provisioning, container/Docker host resources, Docker containers, host configuration sources, virtual/host networking resources, and local volumes that are not already owned by core Resource Manager UI. | These registrations are metadata only; provider-specific create/edit experiences should be designed as graph declaration/deployment authoring instead of copying old provider stores. |
| Container app Deployment, Revisions, Scale and replicas, Monitoring, and endpoint actions | Provider-unique UI is selected by `application.container-app` for graph-backed resources. Deployment, monitoring, and endpoint actions use public Resource Manager APIs and projected attributes; revisions can render with an empty history provider until graph/runtime history is ported; scaling uses a graph-safe panel over the public replica update API and lists requested replica slots from Resource Manager projections. | Port rich revision history, deployment records, and runtime-specific details without depending on the old application-provider definition store. |
| Application storage | The Resource Manager UI project now contributes a graph-native storage tab for executable apps, ASP.NET Core projects, container apps, and SQL Server. It reads projected runtime workload mounts when available and otherwise lists graph volume dependencies while clearly marking mount target details as not projected. | Port editable graph volume-consumer payloads and runtime mount materialization summaries after the graph mutation path and provider-owned runtime descriptor projection are stabilized. |
| Application configuration and environment | The Resource Manager UI project contributes read-only graph Configuration and Environment tabs for ported application resource types. Configuration displays projected graph identity, attributes, endpoints, capabilities, operations, and dependencies; Environment renders ASP.NET Core `project.environmentVariables` declarations and redacts sensitive literal values. | Editable graph apply/update UI remains deferred until the graph mutation path and provider/runtime apply seams replace old application update forms. Runtime-derived environment variables are not persisted back into the graph by the UI. |
...
| SQL Server databases | The Resource Manager UI project now contributes a graph-native **Databases** tab selected by `application.sql-server`. It lists `application.sql-database` resources that are projected as children or that reference the SQL Server resource through the current graph dependency representation. | Live SQL inspection, credential/runtime diagnostics, and configuration editing remain outside this graph UI page until those runtime seams move behind provider-owned Control Plane integrations. |
| SQL Server configuration pages | Still tied to old application-provider SQL services. | Decide during SQL provider switch whether configuration editing becomes a graph definition editor, a Control Plane runtime view, or both. |
...
The switch-readiness samples now remove their old side-by-side comparison
toggles as they move to the Resource model provider path. Old provider records
should not appear in switched samples; temporary runtime seams are kept only
where they bridge accepted Resource model state to current Control Plane or
provider behavior.

| Sample lens | Current graph-model coverage | Next migration question |
| --- | --- | --- |
| ProjectReference | ASP.NET Core API and frontend resources now use stable Resource model identities (`application.dotnet-app:project-reference-api` and `application.dotnet-app:project-reference-frontend`) and prove project-to-project calls through service-discovery variables derived from provider-owned resource references, provider-owned service names, endpoint request attributes, start, stop, logs, traces, metrics, health, endpoint projection, state projection, and ResourceDefinition apply diagnostics. ASP.NET Core state, endpoint, observability, and process-output log projection come from the built-in-provider bridge package. Provider-owned service references validate missing targets and expected resource type mismatches without using `DependsOn`. The old application-provider project records, old application provider registration, side-by-side comparison path, and `GraphOnly` setting have been removed from this sample. | Decide whether ASP.NET Core resource service discovery should remain provider-local for the POC or become a reusable runtime projection after more providers need it. Editable add/update authoring UI is deferred until the old provider UI seams are swept into Resource model declarations or replacement deployment authoring. |
| SplitHosting | The split Control Plane host now exposes only the Resource Definitions-backed network resource through the Resource Manager bridge. The old direct Resource Manager comparison record and comparison toggle have been removed, and smoke coverage verifies the separate UI host can render `cloudshell.network:split-sample` through the remote Control Plane client while the Control Plane API returns only the Resource Definitions projection. The supported sample-host launch matrix also authenticates through the sample client-credentials flow and verifies the protected split Control Plane resources API projects the sample resource. | Use this sample to keep validating Resource Manager projections across split hosting before changing public API/client contracts. |
| SettingsAndSecrets | Configuration Store, Secrets Vault, identity provisioning, and ASP.NET Core project definitions can be applied together through stable Resource model identities (`configuration.store:sample-app`, `secrets.vault:sample-app`, and `application.dotnet-app:settings-secrets-api`). Configuration Store and Secrets Vault project Resource Manager endpoints plus `/healthz` health/liveness declarations through provider-bridge coverage, and the sample host carries an app/settings/secrets vertical slice with endpoint projection, count-summary projection, typed app-to-settings/secrets startup dependencies, provider-owned runtime option registration for backing services and seeded runtime data, backing-service startup, Resource Manager-declared API identity projection/provisioning/grants, API process startup, derived CloudShell client environment from `project.references`, and API consumption coverage. The old application, configuration, and secrets provider registrations, old workload records, graph-prefixed resource names, and side-by-side comparison path have been removed from this sample. | Decide how configuration/secrets resources persist runtime-owned data and whether identity needs first-class graph declarations after more provider ports need that shape. Runtime backing services still start local C# service projects through provider-owned seams; replacing those seams without reviving the old provider aggregate remains provider work. |
| ThirdPartyIdentity | The sample host now declares only Resource model identity provisioning, Configuration Store, and ASP.NET Core project resources for the protected Keycloak workload with stable identities (`cloudshell.identity-provisioning:keycloak`, `configuration.store:third-party-identity`, and `application.dotnet-app:keycloak-provisioned-api`). Non-Docker smoke coverage verifies Resource Manager projection and setup operation shape without old provider records. Docker smoke coverage starts the Keycloak-protected Configuration Store plus ASP.NET Core API, executes identity-provider setup through the sample-local Resource Manager bridge, verifies protected configuration reads with a Keycloak-issued resource identity token, and verifies generated Keycloak compose cleanup after the path completes. The ASP.NET Core process runtime has a provider-local environment seam, and this sample wires a sample-local adapter that derives API identity credential variables from the Resource Manager identity declaration and existing Keycloak credential provider without depending on the old application provider. The old application/configuration provider registrations, old workload records, graph-prefixed resource names, and side-by-side comparison path have been removed from this sample. | Decide whether individual resource identity declarations should remain Resource Manager-owned for the POC or gain first-class graph declarations after more provider ports need that shape. Keep the Keycloak credential adapter sample-local until another provider proves a shared resource-identity runtime environment seam. |
| ContainerHost | The sample host now declares only Resource Definitions-backed storage, volume, and SQL Server resources using stable IDs (`cloudshell.storage:local`, `cloudshell.volume:sql-data`, and `application.sql-server:sql-server`). Service-level coverage verifies Resource Manager projection, storage/volume attributes, typed storage dependency, and SQL Server volume-consumer capability. The old application-provider storage, volume, and SQL Server records, old application provider registration, and comparison toggle have been removed while SQL Docker lifecycle coverage runs through the provider-owned local SQL Server Docker runtime adapter. The SQL resource still contributes a Resource Manager orchestration descriptor with `ControlPlaneScoped` lifetime, and smoke coverage verifies graceful host shutdown removes the SQL Docker container through the host-scoped shutdown service. The generic sample-host readiness smoke test still removes the SQL container after launch because that harness kills sample processes instead of running hosted shutdown. | Use this sample to keep validating storage-backed volume semantics before runtime storage materialization moves behind provider seams. Generalized workload descriptor projection, durable provider-backed storage materialization, and default/preferred container-host placement remain provider work. |
| ApplicationTopology | The sample host now declares only Resource Definitions-backed storage, volume, SQL Server/database, Configuration Store, Secrets Vault, host-configuration source, API, frontend, and local DNS resources with stable identities such as `cloudshell.storage:application-topology-local`, `application.sql-server:application-topology-sql-server`, `configuration.store:application-topology-settings`, `secrets.vault:application-topology-secrets`, `application.dotnet-app:application-topology-api`, and `application.dotnet-app:application-topology-frontend`. The old application/configuration/secrets/storage/volume/SQL workload records, side-by-side comparison setting, old SQL Resource Manager bridge, and graph-prefixed sample names have been removed. Smoke coverage verifies Resource Manager projection without old records, local DNS retargeting to the Resource model frontend, local DNS name-mapping reconciliation to a configured hosts file, host-configuration source projection without exposed values, the storage -> volume -> SQL Server -> database chain, Configuration Store and Secrets Vault runtime startup, authenticated entry/secret reads, API settings/secrets consumption through CloudShell clients, runtime identity credential projection from the API identity binding, SQL credential materialization through the provider-owned credential endpoint injected from graph references, API `/database`, frontend-to-API `/upstream`, SQL Docker lifecycle cleanup, and graceful host shutdown cleanup. SQL Docker lifecycle now uses the provider-owned local SQL Server Docker runtime adapter. | Use this sample to keep validating multi-provider dependencies, host-configuration source metadata, SQL child ownership, volume materialization, SQL credential flow, generated Resource Manager UI parity, and application-level discovery while replacing sample-local seams with provider-owned runtime implementations. Reusable SQL grant reconciliation, Resource model environment editing projection, generalized workload descriptor projection, and removal of old provider package bridge dependencies remain provider work. |
| ContainerAppDeployment | The sample host now declares only Resource Definitions-backed Docker host, registry container, and container-app resources with stable IDs (`docker.host:sample`, `docker.container:sample-registry`, and `application.container-app:sample-api`) through provider-owned Resource Model builders. The old Docker/application provider records, old provider registrations, side-by-side comparison path, and `GraphOnly` setting have been removed for this sample. Smoke coverage verifies Resource Manager projection, registry attributes, typed dependencies, deployment image updates, replica updates, deferred lifecycle warning behavior, and generated container-app UI panels against the new resource IDs. Docker container lifecycle operations use the provider-owned local Docker container runtime adapter with Resource Manager state projection and status-aware action availability. The sample keeps registry materialization opt-in behind `ContainerAppDeployment:EnableDockerRuntime`, disabled by default so normal projection does not shell out to Docker. Docker smoke coverage verifies registry start, `/v2/` availability, stop, container cleanup, and graceful host shutdown cleanup without old provider records. The Resource Model bridge can satisfy the existing deployment image-update API and replica update API for container apps by applying `container.image` and/or `container.replicas` changes into the accepted Resource Model state before executing temporary image-update or replica-update operation seams. Those seams are POC adapter hooks for accepted Resource Model changes because the current API still models image deployment and scaling as Control Plane workflows; they do not decide whether image upload, deployment, or non-lifecycle scale workflows should later become explicit resource operations. Lifecycle actions on the deferred bridge intentionally return warnings instead of pretending to start a runtime. | Keep registry materialization opt-in so normal sample projection tests do not slow down or race on Docker CLI availability, then fold container app runtime deployment into the durable orchestrator/controller path. |
| ReplicatedContainerHealth | The sample host now declares only Resource Definitions-backed Docker host and replicated container-app resources with stable identities (`docker.host:sample` and `application.container-app:api`). The old `GraphOnly` comparison seam, old application/Docker provider registrations, old `application:api` resource record, old Resource Manager bridge, and graph-prefixed sample resource names have been removed. Smoke coverage verifies Resource Manager projection, replica-count attributes, typed dependencies, endpoint projection, generated container-app UI tabs, health/liveness declarations, image/replica updates, app-owned ingress cleanup, runtime-scope health aggregation, log source discovery, Docker log reading, running-container observability environment, Resource Manager observability projection for hidden runtime replicas, and live trace/metric ingestion under projected runtime replica resource IDs. Container-app runtime and orchestrator-service dispatch now use the provider-owned local Docker container-app runtime adapter, including Docker/Traefik materialization, hidden runtime replica projection, and provider-owned runtime-container log/monitoring support. | Complete metrics/traces UI parity and richer Resource Manager state projection without reintroducing the old application-provider aggregate. Generated details telemetry tabs for hidden runtime replica resources need a runtime-resource-specific details path, and the durable runtime should represent container startup separately so newly-started replicas can show "starting" while health converges. |
| HostVirtualNetwork and LoadBalancer | Network, virtual network, host networking, DNS zone, name mapping, service, and load-balancer providers have definition/projection coverage. HostVirtualNetwork now carries only Resource Definitions-backed local host networking, ASP.NET Core target API, virtual-network, DNS zone, and name-mapping resources in the sample host; the old application-provider registration plus direct Resource Manager comparison records, comparison toggle, and sample-local endpoint-mapping bridge have been removed while endpoint-mapping reconciliation runs through the provider-owned graph reconciler and the Control Plane local host-network provisioner. CoreDNS zone-file publishing is an opt-in provider-owned adapter. LoadBalancer now carries only Resource Definitions-backed Docker host, container-app targets, load-balancer, DNS zone, and name-mapping resources with stable sample identities such as `docker.host:sample-host`, `application.container-app:api`, `cloudshell.loadBalancer:public`, and `cloudshell.dnsZone:cloudshell-local`. The old Docker/application/load-balancer/DNS resource records, provider registrations, graph-prefixed sample names, and comparison toggle have been removed while route generation now runs through the Traefik provider's Resource Model load-balancer configuration applier and DNS publishing runs through the provider-owned graph DNS reconciler. Smoke coverage verifies Resource Manager projection, typed dependencies, load-balancer count/operation shape, virtual-network endpoint mapping runtime handoff, HostVirtualNetwork switch readiness without old records, load-balancer dynamic configuration, DNS publishing to a temporary hosts file, name-mapping projection without old application records, and LoadBalancer switch readiness without sample-local load-balancer applier code. | Continue validating endpoint mapping operations, route payloads, and provider-owned network/load-balancer reconciliation as the graph model starts carrying richer route and backend configuration. |
...
| Register an ASP.NET Core Resource model resource in a real host/sample | Done | ProjectReference declares `Project Reference API` through the Resource Manager bridge provider. |
...
| Decide minimal runtime state projection | Done | The bridge can accept optional runtime-state providers. `null` remains the neutral no-status case, lifecycle-capable graph resources fall back to `Unknown`, and observed runtime state can enable actions such as Restart without putting runtime loops into the graph model. Container-app graph state now projects through the provider-owned runtime handler seam and bridge state provider. |
...
manual `ResourceGraphBuilder` builder, plus any apply, update, or
...
A narrow container application built-in provider extends the same proof
...
attributes, start/restart operation providers, typed image-update and
replica-update operation providers, and a typed projection wrapper. Explicit
host binding is expressed
...
capability. The image-update and replica-update operations stage resource-local
attribute changes on the attached `Resource`; the container application type provider then
...
storage-backed SQL data volume, an explicit SQL Server endpoint, and SQL Server
consuming that volume while declaring the host relationship as a typed
`ResourceReference` when a host is selected. The SQL provider now wires the
lifecycle operation seam to a provider-owned local Docker runtime adapter that
resolves the SQL resource's graph dependency closure, locates the mounted
CloudShell volume and storage parent, and converts that accepted graph state
into the runtime bind mount. That proves the graph-to-runtime boundary for
storage-backed volume consumption while keeping local Docker behavior behind a
provider adapter. Docker smoke coverage now exercises the SQL
start/restart/stop path through Resource Manager, verifies the storage-backed
volume directory is created, and verifies the runtime container is removed on
stop.

An ASP.NET Core project built-in provider extends the proof without depending
on `CloudShell.Providers.Applications`. It owns
`application.dotnet-app`, project path and launch-related attributes,
...
built-in-provider companion projects effectively carry the new Control
Plane/Resource Manager provider integration role, while
`CloudShell.ControlPlane.Providers.UI` carries
...
A narrow host configuration source built-in provider covers the companion
...
A narrow Docker host built-in provider covers the provider-specific
...
`cloudshell.container-host` built-in provider so Docker-owned runtime services
and attributes can evolve inside the Docker provider boundary.

A narrow load balancer built-in provider covers the declarative
...
A narrow network built-in provider covers the declarative `cloudshell.network`
...
A narrow virtual network built-in provider covers the more specific
...
reconciler with a no-op POC default. The reconciler receives the resource
projection execution context so runtime/provider implementations can inspect
graph-scoped target and provider resources while the graph model stays focused
on declaration and projection. Observed endpoint mapping state remains a
future capability member or operation plan rather than a normal count
attribute.

The HostVirtualNetwork sample now uses the provider-owned
`ResourceModelGraphEndpointMappingReconciler` for this seam. The reconciler
projects Resource Definitions-backed resources through the Resource Manager
endpoint projection providers and delegates materialization to the Resource
Manager endpoint mapping provisioner. Host proxy runtime behavior remains
owned by the Control Plane local host-network provisioner instead of moving
into the graph provider itself. Smoke coverage starts the ASP.NET Core target,
invokes the virtual network reconcile action through the Control Plane API, and
verifies the public ingress forwards to the target health endpoint.

After the provider switch, runtime-specific networking code should be swept
and isolated according to runtime boundaries. Runtime may be part of Resource
Manager, or the glue inside Resource Manager that performs advanced host and
orchestration behavior, but macOS-specific host-networking behavior should not
live in the graph provider shape itself.

A narrow DNS Zone built-in provider covers the declarative
...
mapping reconciler with a no-op POC default. The reconciler receives the
resource projection execution context so runtime/provider implementations can
inspect sibling graph resources, such as name mappings declared next to the
zone, while keeping hosts-file or other DNS materialization behind the
Resource Manager/provider boundary. The POC
...
A narrow name-mapping built-in provider covers the declarative
...
A narrow storage built-in provider covers the declarative `cloudshell.storage`
...
A narrow CloudShell volume built-in provider covers the declarative
...
A narrow service built-in provider covers the declarative `cloudshell.service`
...
```jsonc
{
  ...
  "target": {
    "resource": {
      "value": "application.dotnet-app:api",
      "relationship": "dependsOn",
      "addressingMode": "resourceId",
...
A narrow Secrets Vault built-in provider follows the same boundary while
...
A narrow identity provisioning built-in provider covers
...
A narrow local host networking built-in provider covers
...
A narrow macOS host networking built-in provider covers
...
A narrow Docker container built-in provider covers `docker.container` as a
...
| --- | --- | --- | --- |
| Executable application (`application.executable`) | Modeled as a built-in provider | Type and class defaults, executable path validation and command configuration, shared volume-consumer capability, provider-declared default console log source, programmatic builder activation for runtime monitoring, start operation with an injected provider-owned process runtime controller that honors configured arguments and working directory, typed wrapper, Resource Manager bridge projection, execution, and process monitoring snapshots | Control Plane log read/stream integration, stop/restart lifecycle operations, state-sensitive action availability, endpoints, templates, and UI registration/update flow |
| Local volume (`storage.volume`) | Modeled as a built-in provider | Storage class and type defaults, medium validation, provision operation with an injected provider-owned provisioner seam, typed wrapper, apply planning, Resource Manager bridge projection | Provider-backed storage materialization, usage tracking, health, and monitoring |
| Storage (`cloudshell.storage`) | Modeled as a narrow built-in provider | Storage class/type defaults, provider/medium/location attributes, passive storage-provider and mount-provider capability markers, inspect operation with an injected provider-owned inspector seam, typed wrapper, apply planning, and Resource Manager bridge projection/execution | Volume collection payloads, runtime filesystem availability and volume counts as capability members or operation plans, provider-backed storage materialization, health, monitoring, and UI registration/update flow |
| CloudShell volume (`cloudshell.volume`) | Modeled as a narrow built-in provider | Storage class/type defaults, provider/medium/location/subpath/access-mode/persistence attributes, passive storage-volume capability marker, typed `ResourceReference` storage dependencies, storage-reference graph validation, type-specific `storage.volume.provision` operation provider with an injected provider-owned provisioner seam, typed wrapper, apply planning, and Resource Manager bridge projection/execution | Runtime filesystem availability as capability members or operation plans, provider-backed volume materialization, health, monitoring, and UI registration/update flow |
| Service (`cloudshell.service`) | Modeled as a narrow built-in provider | Service class/type defaults, service kind/routing-mode attributes, passive endpoint-source capability marker, temporary typed `ResourceReference` target/network dependencies, target/network graph validation, reconcile operation with an injected provider-owned reconciler seam, typed wrapper, apply planning, and Resource Manager bridge projection/execution | A clear product/runtime use case. `cloudshell.service` is a potential service boundary that may later map to an orchestration service, but it is not used by current samples. Port, endpoint, and health-check payloads, provider-specific reference modeling, endpoint projection through Resource Manager, runtime routing/materialization, richer target eligibility validation, orchestration integration, and UI registration/update flow are deferred until that case is clear. |
| Container application (`application.container-app`) | Modeled as a narrow built-in provider | Image, registry, and replica attributes, optional typed generic/Docker container-host reference validation/projection, shared volume-consumer capability, start/stop/restart/image-update/replica-update operations with an injected provider-owned runtime handler seam, typed wrapper, ContainerAppDeployment and ReplicatedContainerHealth sample-inspired graph coverage, Resource Manager bridge projection, endpoint projection, execution, ContainerAppDeployment image/replica update API coverage through a sample-local graph-state bridge without old provider records, and ReplicatedContainerHealth Docker runtime coverage | Actual container host orchestration through a runtime handler implementation, revisions, replica runtime state, monitoring, and UI operations |
| ASP.NET Core project (`application.dotnet-app`) | Modeled as a narrow built-in provider | Project path, arguments, hot reload, launch-settings attributes, provider-owned typed endpoint request and environment-variable attributes, optional provider-owned service-discovery name, provider-owned graph references for developer service discovery with missing-target and expected-type graph validation, shared volume-consumer capability, provider-declared default console log source, start/stop/restart operations with state-sensitive runtime availability and a provider-owned process runtime controller that can derive `--urls` from endpoint requests, match the old provider's non-interactive `dotnet watch` and non-hot-reload build-before-run process semantics, derive Aspire-style service-discovery variables from graph references, endpoint requests, and selected provider-owned service endpoint attributes, derive CloudShell Configuration Store, Secrets Vault, and SQL Server credential client environment from referenced graph resources, and apply non-secret environment variables, typed wrapper, Resource Manager bridge projection/execution, bridge-package state/endpoint/observability/process-output log projection, graph-to-graph project call coverage, log, metric, trace, health, Resource Manager details-tab coverage, launcher-backed ProjectReference startup and graph-to-graph service discovery coverage without old provider records, SettingsAndSecrets graph-backed API coverage consuming graph-backed settings/secrets through reference-derived environment including graph-only mode without old provider records, Resource Manager-declared graph API identity grants, provider-local runtime seams, ThirdPartyIdentity graph-only Keycloak credential environment coverage, ApplicationTopology-inspired coverage for configurable SQL Server, Configuration Store, and Secrets Vault `project.references` without using `DependsOn` for discovery, including graph API `/database` through provider-owned SQL credentials, and graph-safe Resource Manager UI metadata registration for generated views in graph-only samples | Launch settings parsing, richer process diagnostics, reusable service-discovery/environment-variable conventions if later providers prove the need, first-class graph identity/provisioning projection if the POC proves it belongs in the graph model, reusable graph SQL credential/grant integration, container build behavior, editable UI registration/update flow |
| SQL Server (`application.sql-server`) | Modeled as a narrow built-in provider | Service class and type defaults, version/edition attributes, provider-owned typed endpoint request attribute, optional typed generic/Docker container-host reference validation/projection, declared database configuration, shared volume-consumer capability over direct and storage-backed volumes, start/stop/restart lifecycle operation declarations with an injected provider-owned runtime handler seam and Resource Manager bridge state projection, provider-owned local SQL Server Docker runtime adapter for mapped resources, reconcile-access operation with an injected provider-owned runtime reconciler seam, typed wrapper, ContainerHost sample-inspired graph coverage, ASP.NET Core service-discovery environment projection from explicit SQL Server `project.references`, Resource Manager bridge projection, endpoint contract/network mapping projection, execution, provider-owned graph SQL credential materialization, smoke coverage that resolves storage-backed volume mounts from the graph without old provider records and verifies host-scoped graceful shutdown, and graph-safe Resource Manager UI metadata registration for generated views in graph-only samples | Default/preferred container-host resolution, generalized workload descriptor projection beyond local executable mappings, reusable provider grant reconciliation beyond the current on-demand credential endpoint, database child projections, and editable/provider-specific UI tabs |
| SQL database child (`application.sql-database`) | Modeled as a narrow built-in provider | Database name/source/ensure-created attributes, provider-managed read-only `database.server` `ResourceReference` attribute declaration for the owning SQL Server, temporary server `ResourceReference` validation through current `DependsOn` inputs, typed wrapper projection of the owning server as a `belongsTo` reference, ensure-created operation with an injected provider-owned runtime creation handler seam that receives both database and resolved owning SQL Server resources, Resource Manager bridge projection and execution, and provider-owned Resource model SQL database creation against the configured SQL endpoint without storing administrator credentials in graph state | Provider projection of `database.server` when SQL database children are materialized by the SQL provider, credential/grant reconciliation, provider-managed child ownership metadata, and UI tabs |
| Container host (`cloudshell.container-host`) | Modeled as a narrow built-in provider | Infrastructure class/type defaults, host kind/endpoint/registry/default attributes, passive container image/build/filesystem-mount capability markers, inspect operation with an injected provider-owned inspector seam, typed wrapper, Resource Manager bridge projection and execution | Real Docker/container host runtime integration, host resolution, placement behavior, credentials, and runtime diagnostics |
| Docker host (`docker.host`) | Modeled as a narrow built-in provider | Infrastructure class/type defaults, Docker host kind/endpoint/registry/default attributes, passive container image/build/filesystem-mount capability markers, inspect operation with an injected provider-owned inspector seam, typed wrapper, Resource Manager bridge projection and execution | Real Docker runtime integration, discovery, health, logs, container child projections, credentials, and UI registration/update flow |
| Docker container (`docker.container`) | Modeled as a narrow built-in provider | Container class/type defaults, workload/image/registry/replica attributes, read-only endpoint-count attribute, passive monitoring and log-source capability markers, lifecycle operation projections, typed wrapper, apply planning, and Resource Manager bridge projection/execution | Real Docker API integration, runtime discovery, container state, state-sensitive action availability, log streaming, endpoint projection, and hidden/runtime-managed Resource Manager behavior |
| Load balancer (`cloudshell.loadBalancer`) | Modeled as a narrow built-in provider | Network class/type defaults, provider/host attributes, read-only count attributes, provider-owned entrypoint and route payload shapes, passive networking capability markers, temporary typed host/backend `ResourceReference` dependencies, backend-target graph validation, apply-configuration operation with an injected provider-owned applier seam, typed wrapper, Resource Manager bridge projection/execution, Resource Manager projection of frontends, endpoint mappings, and route collections from graph payloads, LoadBalancer sample coverage that translates declared routes into the existing Traefik provider context so the provider-owned configuration writer handles Resource model resources consistently, and LoadBalancer switch-readiness coverage without the old application-provider resource records | Provider-specific reference modeling if needed, Traefik runtime container materialization and UI registration/update flow |
| Network (`cloudshell.network`) | Modeled as a narrow built-in provider | Network class/type defaults, kind/readiness/provider attributes, passive networking capability markers, reconcile-endpoint-mappings operation with a context-aware injected provider-owned reconciler seam, typed wrapper, Resource Manager bridge projection and execution | Endpoint and mapping payloads, observed mapping state as capability members, host/virtual network specialization, provisioner integration, and UI registration/update flow |
| Virtual network (`cloudshell.virtualNetwork`) | Modeled as a narrow built-in provider | Network class/type defaults, virtual/default/readiness/provider attributes, provider-owned endpoint, endpoint-network-mapping, and endpoint-mapping payload attributes, passive virtual-network and ingress capability markers, type-specific `reconcileEndpointMappings` operation provider with a context-aware injected provider-owned reconciler seam, typed wrapper, apply planning, Resource Manager bridge projection/execution, provider-owned graph endpoint-mapping reconciler integration, HostVirtualNetwork sample public-ingress mapping coverage, and public-ingress traffic verification through the Control Plane action path without old provider records | Observed mapping state as capability members or operation plans and UI registration/update flow |
| Local host networking (`cloudshell.hostNetworking.local`) | Modeled as a narrow built-in provider | Infrastructure class/type defaults, host-readiness/OS/mode attributes, passive networking provider/endpoint-mapper/gateway/ingress/host-network capability markers, type-specific `reconcileEndpointMappings` operation provider with a context-aware injected provider-owned reconciler seam, typed wrapper, apply planning, and Resource Manager bridge projection/execution | Live mapping counts, host proxy runtime state, endpoint mapping provisioner integration, macOS-specific runtime isolation, diagnostics, and UI registration/update flow |
| macOS host networking (`cloudshell.hostNetworking.macos`) | Modeled as a narrow built-in provider | Infrastructure class/type defaults, host-readiness/OS/mode attributes, passive networking provider/endpoint-mapper/gateway/ingress/host-network capability markers, type-specific `reconcileEndpointMappings` operation provider with a context-aware injected provider-owned reconciler seam, typed wrapper, apply planning, and Resource Manager bridge projection/execution | Platform support checks, live mapping counts, host proxy runtime state, endpoint mapping provisioner integration, macOS-specific runtime isolation, diagnostics, and UI registration/update flow |
| DNS Zone (`cloudshell.dnsZone`) | Modeled as a narrow built-in provider | Network class/type defaults, zone/provider attributes, passive DNS-zone capability marker, reconcile-name-mappings operation with a context-aware injected reconciler seam, typed wrapper, Resource Manager bridge projection/execution, provider-owned graph DNS name-mapping reconciler integration, and LoadBalancer/HostVirtualNetwork DNS zone coverage that delegates Resource model name mappings to the existing Resource Manager name-publishing provider contract | Record/conflict/materialization views as capability members or operation plans, DNS publisher integration, and UI registration/update flow |
| Name mapping (`cloudshell.nameMapping`) | Modeled as a narrow built-in provider | Network class/type defaults, host/endpoint/exposure attributes, provider-managed unset materialization-status attribute, passive name-mapping capability marker, temporary `ResourceReference` DNS-zone and target dependencies, typed wrapper, apply planning, graph validation, Resource Manager bridge projection, LoadBalancer sample name mappings targeting the load-balancer frontend, and provider-owned graph DNS reconciler coverage that materializes Resource model name mappings through the existing local-hostnames publisher contract | Provider-specific reference modeling if needed, target endpoint validation, conflict/materialization views as capability members or operation plans, DNS publisher integration, and UI registration/update flow |
| Configuration store (`configuration.store`) | Modeled as a narrow built-in provider | Configuration class/type defaults, endpoint and read-only entry-count summary attributes, `/healthz` health/liveness declarations, start/stop/restart operations backed by a provider-local process controller, provider-owned runtime entry seed data, inspect operation with an injected provider-owned inspector seam and default runtime-backed entry-count diagnostics, typed wrapper, Resource Manager bridge projection/execution, optional Resource Manager endpoint projection through a companion provider-bridge package, SettingsAndSecrets smoke coverage for endpoint, inspect-action projection/execution, authorized entry reads, and API consumption through the Resource model endpoint, and ThirdPartyIdentity runtime coverage with external Keycloak service-bearer validation | Durable entry storage, logs, templates, and UI registration/update flow |
| Host configuration source (`configuration.host`) | Modeled as a narrow built-in provider | Configuration class/type defaults, source and read-only entry-count attributes, inspect operation with an injected provider-owned inspector seam, typed wrapper, Resource Manager bridge projection and execution, and ApplicationTopology projection coverage without exposing host configuration values | Runtime host configuration lookup, entry-name payloads, authorization, templates, and UI registration/update flow |
| Secrets Vault (`secrets.vault`) | Modeled as a narrow built-in provider | Secrets Vault class/type defaults, endpoint and read-only secret-count summary attributes, `/healthz` health/liveness declarations, start/stop/restart operations backed by a provider-local process controller, provider-owned runtime secret seed data, inspect operation with an injected provider-owned inspector seam and default runtime-backed secret-count diagnostics without exposing values, typed wrapper, Resource Manager bridge projection/execution without storing secret values in graph attributes, optional Resource Manager endpoint projection through a companion provider-bridge package, and SettingsAndSecrets smoke coverage for endpoint, inspect-action projection/execution, authorized secret reads, and API consumption through the graph-backed endpoint | Durable secret storage, logs, templates, and UI registration/update flow |
| Identity provisioning (`cloudshell.identity-provisioning`) | Modeled as a narrow built-in provider | Infrastructure class/type defaults, provider/provider-id/provider-kind attributes, passive identity-provisioning capability marker, setup operation with an injected provider-owned setup handler seam, typed wrapper, apply planning, Resource Manager bridge projection/execution, ThirdPartyIdentity sample bridge coverage that delegates setup to the Resource Manager identity setup service for the attached provider, Docker-backed sample execution against the Keycloak setup handler, and Keycloak setup coverage without old identity/application/configuration provider records | Directory/client materialization, credential issuance, grant reconciliation, authorization, provider-owned diagnostics, and UI registration/update flow beyond the setup seam |
...
| ReplicatedContainerHealth | Resource model container app image/replica updates, app-owned ingress, health/liveness refresh, hidden runtime replica projection, logs, traces, metrics, and cleanup. |
| ThirdPartyIdentity | Resource model identity provisioning, Keycloak setup, protected configuration service, and API configuration reads without old provider records. |
| SettingsAndSecrets | Resource model Configuration Store and Secrets Vault runtime services, authorized entry/secret reads, API startup, and client environment projection without old provider records. |
| ProjectReference | ASP.NET Core project startup and project-to-project service discovery without old provider records. |
| HostVirtualNetwork | Endpoint-mapping reconciliation through the provider-owned graph reconciler and public ingress reachability. |
| LoadBalancer | Resource model load-balancer, DNS/name-mapping projection without old application records, and Traefik-backed routing paths. |
| ContainerHost and ContainerAppDeployment | Resource model storage/SQL container-host path, registry runtime, and container-app state/update flows without old provider records for the switched samples. |
...
Manager resource, authenticates against the protected SplitHosting Control
Plane resources API, then removes known graph runtime artifacts from samples
...
| `ProjectReferenceLauncher_RunsProjectsWithoutOldProviderRecords` | Proves launcher-backed ASP.NET Core project startup and project-to-project service discovery without old application records. |
| `SettingsAndSecretsSample_RunsServicesAndApiWithoutOldProviderRecords` | Proves Configuration Store, Secrets Vault, API startup, authorized reads, and client environment projection without old provider records. |
| `ThirdPartyIdentitySample_KeycloakProvisionedWorkloadReadsConfiguration` | Proves identity provisioning, configuration service bearer validation, API identity environment, and Keycloak cleanup without old provider records. |
| `ApplicationTopologyHost_RunsSqlBackedWorkload` | Proves the multi-provider graph workload: storage-backed SQL, database materialization, settings/secrets, API, frontend, and graph service discovery. |
| `ReplicatedContainerHealthSample_ImageAndReplicaUpdatesRestartContainers` | Proves Resource model container-app runtime behavior, image/replica updates, ingress cleanup, health/liveness, hidden runtime replica resources, logs, traces, and metrics. |
| `ContainerHostSample_SqlRuntimeStartsWithStorageBackedVolume` | Proves the ContainerHost storage/volume/SQL runtime bridge and storage-backed bind-mount materialization. |
| `ContainerAppDeploymentSample_UpdatesContainerAppState` | Proves the existing image and replica update APIs can apply changes to Resource model container apps. |
| `ContainerAppDeploymentSample_StartsRegistryRuntime` | Proves the opt-in registry runtime seam and cleanup path. |
| `HostVirtualNetworkSample_ReconcilesEndpointMappingThroughRuntimeBridge` | Proves endpoint mapping reconciliation through the provider-owned graph reconciler. |
| `LoadBalancerSample_RunsLoadBalancerAndDnsPathsWithoutOldProviderRecords` | Proves Resource model load-balancer, route generation, DNS zone, and name-mapping paths without old application records. |
...
Provider-owned Resource Model runtime adapters are composed through
`UseBuiltInResourceModelRuntimeAdapters(...)` on the Control Plane builder
or `AddBuiltInResourceModelRuntimeAdapters(...)` on `IServiceCollection`.
This preset is intentionally separate from resource type registration: it
installs current Resource Model-to-runtime adapters such as endpoint-mapping
and DNS name-mapping reconciliation only when a host wants local runtime
materialization. Individual adapter registration methods remain available so
future split provider packages can expose their own narrow setup APIs without
depending on the aggregate built-in package.
...
built-in providers during the migration, while application workload,
networking, storage, and infrastructure providers may still split into more
...
The current Control Plane contract exposes `GetResources()` as a flat list,
which means providers that need to expose hidden runtime children or other
sub-resources must currently return them beside top-level resources and rely on
`ParentResourceId`, `ManagementMode`, and `Visibility` metadata. During the POC
we should keep using that adapter shape instead of creating another provider
abstraction. After sample switch-over, the provider contract should be
re-evaluated as a query surface rather than only a flat enumeration method.
The default query should list top-level provider resources. Callers should be
able to opt into sub-resources, projected runtime resources that are not
persisted as graph records, or a grouped shape that keeps sub-resources under
their owning resource. Providers that create runtime artifacts or projected
resources should be responsible for tracking the objects they own closely
enough to answer those queries and to clean them up. Authorization, resource
visibility, and UI choices about whether to display sub-resources can then be
applied on top of the provider query result instead of being baked into the
provider's storage model.
...
more consistent. For the immediate POC, some built-in providers still use
...
The current POC uses `inspect` on several built-in providers as a tentative
