...
| SQL Server (`application.sql-server`) | Modeled as a narrow built-in provider | Service class and type defaults, version/edition attributes, provider-owned typed endpoint request attribute, optional typed generic/Docker container-host reference validation/projection, declared database configuration, shared volume-consumer capability over direct and storage-backed volumes, start/stop/restart lifecycle operation declarations with an injected provider-owned runtime handler seam and Resource Manager bridge state projection, reconcile-access operation with an injected provider-owned runtime reconciler seam, typed wrapper, ContainerHost sample-inspired graph coverage, ASP.NET Core service-discovery environment projection from explicit SQL Server `project.references`, Resource Manager bridge projection, endpoint contract/network mapping projection, execution, ApplicationTopology sample-local graph SQL lifecycle adapter with cached status projection, ApplicationTopology sample-local graph SQL credential materialization, ContainerHost sample-local SQL Docker lifecycle bridge plus smoke coverage that resolves storage-backed volume mounts from the graph without old provider records and verifies host-scoped graceful shutdown through a sample-local Resource Manager workload descriptor, and graph-safe Resource Manager UI metadata registration for generated views in graph-only samples | Real SQL runtime integration behind the lifecycle runtime handler, default/preferred container-host resolution, generalized workload descriptor projection outside the sample-local bridge, reusable credential/grant reconciliation outside the sample-local endpoint, database child projections, and editable/provider-specific UI tabs |
