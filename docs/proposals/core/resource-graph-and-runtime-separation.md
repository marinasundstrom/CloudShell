...
| ContainerAppDeployment | The sample host now declares only Resource Definitions-backed Docker host, registry container, and container-app resources with stable IDs (`docker.host:sample`, `docker.container:sample-registry`, and `application.container-app:sample-api`) through provider-owned Resource Model builders. The old Docker/application provider records, old provider registrations, side-by-side comparison path, and `GraphOnly` setting have been removed for this sample. Smoke coverage verifies Resource Manager projection, registry attributes, typed dependencies, deployment image updates, replica updates, and generated container-app UI panels against the new resource IDs. Docker container lifecycle operations have a provider-owned runtime handler seam with Resource Manager state projection and status-aware action availability. The sample carries an opt-in registry materializer for `docker.container:sample-registry` behind `ContainerAppDeployment:EnableDockerRuntime`, disabled by default so normal projection does not shell out to Docker. The materializer uses a sample-local Docker command runner with deterministic status and lifecycle command coverage, bounded cached status projection, and Docker smoke coverage that verifies registry start, `/v2/` availability, stop, container cleanup, and graceful host shutdown cleanup without old provider records. The Resource Model bridge can satisfy the existing deployment image-update API and replica update API for container apps by applying `container.image` and/or `container.replicas` changes into the accepted Resource Model state before executing temporary image-update or replica-update operation seams. Those seams are POC adapter hooks for accepted Resource Model changes because the current API still models image deployment and scaling as Control Plane workflows; they do not decide whether image upload, deployment, or non-lifecycle scale workflows should later become explicit resource operations. | Port runtime deployment only when the Resource Model-to-Resource Manager runtime apply boundary can be tested against container-host seams without reintroducing the old application-provider aggregate. Keep registry materialization opt-in so normal sample projection tests do not slow down or race on Docker CLI availability. |
| ReplicatedContainerHealth | The sample host now declares only Resource Definitions-backed Docker host and replicated container-app resources with stable identities (`docker.host:sample` and `application.container-app:api`). The old `GraphOnly` comparison seam, old application/Docker provider registrations, old `application:api` resource record, old Resource Manager bridge, and graph-prefixed sample resource names have been removed. Smoke coverage verifies Resource Manager projection, replica-count attributes, typed dependencies, endpoint projection, generated container-app UI tabs, health/liveness declarations, image/replica updates, app-owned ingress cleanup, runtime-scope health aggregation, log source discovery, Docker log reading, running-container observability environment, Resource Manager observability projection for hidden runtime replicas, and live trace/metric ingestion under projected runtime replica resource IDs. Container-app runtime and orchestrator-service dispatch now use the provider-owned delegating runtime handler with a sample-local runtime target. The sample still carries sample-local Docker/Traefik materialization, log, monitoring, and hidden-runtime-resource providers until the durable provider runtime owns replica materialization and projection. | Complete metrics/traces UI parity and richer Resource Manager state projection without reintroducing the old application-provider aggregate. Generated details telemetry tabs for hidden runtime replica resources need a runtime-resource-specific details path, and the durable runtime should represent container startup separately so newly-started replicas can show "starting" while health converges. |
| HostVirtualNetwork and LoadBalancer | Network, virtual network, host networking, DNS zone, name mapping, service, and load-balancer providers have definition/projection coverage. HostVirtualNetwork now carries only Resource Definitions-backed local host networking, ASP.NET Core target API, and virtual-network resources in the sample host; the old application-provider registration plus direct Resource Manager comparison records, comparison toggle, and sample-local endpoint-mapping bridge have been removed while endpoint-mapping reconciliation runs through the provider-owned graph reconciler and the Control Plane local host-network provisioner. LoadBalancer now carries only Resource Definitions-backed Docker host, container-app targets, load-balancer, DNS zone, and name-mapping resources with stable sample identities such as `docker.host:sample-host`, `application.container-app:api`, `cloudshell.loadBalancer:public`, and `cloudshell.dnsZone:cloudshell-local`. The old Docker/application/load-balancer/DNS resource records, provider registrations, graph-prefixed sample names, and comparison toggle have been removed while route generation remains active through a sample-local Traefik bridge and DNS publishing runs through the provider-owned graph DNS reconciler. Smoke coverage verifies Resource Manager projection, typed dependencies, load-balancer count/operation shape, virtual-network endpoint mapping runtime handoff, HostVirtualNetwork switch readiness without old records, load-balancer dynamic configuration, DNS publishing to a temporary hosts file, name-mapping projection without old application records, and LoadBalancer switch readiness through the remaining sample-local Traefik bridge. | Continue validating endpoint mapping operations, route payloads, and provider-owned network/load-balancer reconciliation as the graph model starts carrying richer route and backend configuration. |
...
`ResourceReference` when a host is selected. The sample now wires the SQL
lifecycle operation seam to a sample-local Docker bridge that resolves the
SQL resource's graph dependency closure, locates the mounted CloudShell volume
and storage parent, and converts that accepted graph state into the runtime
bind mount. That proves the graph-to-runtime boundary for storage-backed
volume consumption without promoting the sample bridge into a generic storage
or Docker runtime design. Docker smoke coverage now exercises the SQL
...
| HostVirtualNetwork | Endpoint-mapping reconciliation through the sample runtime bridge and public ingress reachability. |
...
| `HostVirtualNetworkSample_ReconcilesEndpointMappingThroughRuntimeBridge` | Proves endpoint mapping reconciliation through the sample runtime bridge. |
