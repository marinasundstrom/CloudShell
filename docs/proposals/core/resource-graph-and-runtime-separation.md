...
| HostVirtualNetwork and LoadBalancer | Network, virtual network, host networking, DNS zone, name mapping, service, and load-balancer providers have definition/projection coverage. HostVirtualNetwork now carries only Resource Definitions-backed local host networking, ASP.NET Core target API, and virtual-network resources in the sample host; the old application-provider registration plus direct Resource Manager comparison records and comparison toggle have been removed while the endpoint-mapping runtime bridge remains sample-local. LoadBalancer now carries only Resource Definitions-backed Docker host, container-app targets, load-balancer, DNS zone, and name-mapping resources with stable sample identities such as `docker.host:sample-host`, `application.container-app:api`, `cloudshell.loadBalancer:public`, and `cloudshell.dnsZone:cloudshell-local`. The old Docker/application/load-balancer/DNS resource records, provider registrations, graph-prefixed sample names, and comparison toggle have been removed while route generation and DNS publishing remain active through sample-local runtime bridges. Smoke coverage verifies Resource Manager projection, typed dependencies, load-balancer count/operation shape, virtual-network endpoint mapping runtime handoff, HostVirtualNetwork switch readiness without old records, load-balancer dynamic configuration, DNS publishing to a temporary hosts file, name-mapping projection without old application records, and LoadBalancer switch readiness through sample-local runtime bridges. | Continue validating endpoint mapping operations, route payloads, and provider-owned network/load-balancer reconciliation as the graph model starts carrying richer route and backend configuration. |
...
They can be authored programmatically through the resource graph builder today,
or through interchange formats such as JSON, YAML, or XML later. The format
should not change the meaning of the graph declaration.
...
The HostVirtualNetwork sample now supplies the first runtime bridge for this
seam. The bridge is intentionally sample-local: it projects Resource
Definitions-backed resources through the Resource Manager endpoint projection
providers, adapts the local-host networking resource identity to the existing
local-host provisioner contract, and delegates materialization to the Resource
Manager endpoint mapping provisioner. This demonstrates the intended glue
boundary without moving host proxy runtime behavior into the graph provider
itself. Smoke coverage starts the ASP.NET Core target, invokes the virtual
network reconcile action through the Control Plane API, and verifies the public
ingress forwards to the target health endpoint.
...
| DNS Zone (`cloudshell.dnsZone`) | Modeled as a narrow built-in provider | Network class/type defaults, zone/provider attributes, passive DNS-zone capability marker, reconcile-name-mappings operation with a context-aware injected reconciler seam, typed wrapper, Resource Manager bridge projection/execution, LoadBalancer sample DNS zone coverage, and LoadBalancer sample-local runtime bridge coverage that delegates Resource model name mappings to the existing Resource Manager name-publishing provider contract | Generalized name-mapping child resource runtime integration outside the sample bridge, record/conflict/materialization views as capability members or operation plans, DNS publisher integration, and UI registration/update flow |
| Name mapping (`cloudshell.nameMapping`) | Modeled as a narrow built-in provider | Network class/type defaults, host/endpoint/exposure attributes, provider-managed unset materialization-status attribute, passive name-mapping capability marker, temporary `ResourceReference` DNS-zone and target dependencies, typed wrapper, apply planning, graph validation, Resource Manager bridge projection, LoadBalancer sample name mappings targeting the load-balancer frontend, and sample-local runtime bridge coverage that materializes Resource model name mappings through the existing local-hostnames publisher contract | Provider-specific reference modeling if needed, target endpoint validation, conflict/materialization views as capability members or operation plans, generalized DNS publisher integration outside the sample bridge, and UI registration/update flow |
