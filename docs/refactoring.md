...
- [ ] Define a provider execution boundary for host-local runtime work. The
  ...
  - [x] Route ASP.NET Core project lifecycle execution through the provider
    ...
    execution.
  - [x] Route JavaScript app lifecycle execution through the provider
    execution dispatcher with in-process start, stop, and restart handlers,
    moving the local Node.js process sample onto the same provider execution
    boundary while preserving existing action availability checks.
  - [x] Route Java app lifecycle execution through the provider execution
    dispatcher with in-process start, stop, and restart handlers, moving the
    local Java process sample onto the same provider execution boundary while
    preserving existing action availability checks.
  - [x] Route Go app lifecycle execution through the provider execution
    dispatcher with in-process start, stop, and restart handlers, moving the
  ...
  - [x] Inventory direct runtime command paths that should stay behind this
    ...
    setup, SQL Database ensure-created, and Service reconcile controllers now
    have dispatched entry points for their MVP execution paths.
