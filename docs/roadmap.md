...
| 7 | Ecosystem-neutral authoring boundary | CloudShell should not become a C#-only local-development tool, but launchers, deployment artifact loading, and assistant drafting should not distract from MVP stabilization. | Treat the installed CLI, project-local `cloudshell.yaml`, and same-version foreground development host as the MVP path; keep future launchers and SDKs aligned with its ResourceDefinition and Control Plane boundary. |
...
| 9 | Agent/reconciliation preparation | Agents are the future execution boundary, but the MVP should not become a distributed-systems project. | Keep Configuration Store, Secrets Vault, and Device Registry modeled as stable resources while process/container realization remains provider-owned; shape later host-local execution as typed desired/observed work and defer remote agents, placement, regions, and clustering. |
...
5. **Readiness diagnostics before failure.** Before Start, Restart, image
   ...
   The Docker and Apple acceptance slices now prove image-backed lifecycle,
   loopback publication, state/label/network inspection, logs, cleanup, and
   isolated container-to-container communication through provider-reported
   endpoints. A typed owner-scoped runtime boundary now owns common network,
   container, execution, log, and inspection intent, and replica logs/slot
...
15. Advanced app and environment concepts: defer autoscaling, backend pools,
   ...
   project-local YAML, foreground development host, launcher, and SDK boundary. Further
