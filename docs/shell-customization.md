...
- Support CMS-like composition without giving extensions unchecked control of the app. Extensions contribute typed views, hosted view menu items, services, and capabilities; the host owns validation, layout, routing conventions, and ordering.
- Keep Resource-style experiences reusable. Extension-authored hosted views should be able to use the same left-menu and content-host pattern as resource configuration views without depending on Resource Manager.
...
- `AddCustomView` for host-rendered workspaces with CloudShell's common layout.
- `AddCustomViewMenuItem<TComponent>` for menu items inside hosted workspaces.
...
- Consider a widget system for overview/dashboard pages once the hosted view model is stable.
