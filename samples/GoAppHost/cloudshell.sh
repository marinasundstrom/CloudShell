...
app_host_dir="${CLOUDSHELL_GO_APP_HOST_DIR:-$script_dir/AppHost}"
...
control_plane_url="${CLOUDSHELL_CONTROL_PLANE_URL:-http://127.0.0.1:5101}"
app_resource_id="${CLOUDSHELL_APP_RESOURCE_ID:-application.go-app:go-launcher-api}"
...
Commands:
  template       Print the Go-authored ResourceTemplate JSON.
  ...
  start-app      Start the Go launcher-declared app resource.
  stop-app       Stop the Go launcher-declared app resource.
  restart-app    Restart the Go launcher-declared app resource.
...
case "$command" in
  template)
    (cd "$app_host_dir" && run_launcher template "$@")
    ;;
  apply)
    (cd "$app_host_dir" && run_launcher apply "$@")
    ;;
  run)
