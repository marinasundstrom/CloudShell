...
app_host_project="${CLOUDSHELL_APP_HOST_PROJECT:-$script_dir/AppHost/CloudShell.JavaScriptAppHost.csproj}"
...
Commands:
  ...
  reset          Stop recorded host processes and remove generated sample state.
...
case "$command" in
  ...
  reset)
    ...
    rm -rf "$state_dir" "$sibling_state_dir"
    ;;
  open)
    run_cli ui open \
