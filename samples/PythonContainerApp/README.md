# Python Container App Sample

This sample uses the Python launcher to declare a Python app as an
...
`sdk/python/cloudshell`, using the same default credential chain as other
...
```text
http://localhost:5190
http://localhost:5190/configuration
```
...
with the default Python SDK credential chain. The response reports whether the
...
and the local Python SDK package. The container listens on port `8080`;
CloudShell maps it to host port `5190` and starts two local replicas.

## Language and SDK versions

- Python 3.12 is the sample runtime image.
- The app consumes the repository-local `sdk/python/cloudshell` package through
  `PYTHONPATH`; there is no published Python SDK package dependency yet.
