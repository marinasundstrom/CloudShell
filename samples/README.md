...
| `ApplicationTopology` | Multi-resource app topology across storage, SQL, configuration, secrets, identity, DNS, and project resources. | Switched to the Resource model provider path; SQL Docker, SQL credential brokering, configuration, and secrets runtime behavior now use provider-owned adapters. |
