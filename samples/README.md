...
| `ReplicatedContainerHealth` | Replicated container app runtime, health/liveness, logs, traces, metrics, and hidden runtime replica projection. | Switched to the Resource model provider path; container-app runtime/orchestrator dispatch and local command execution now use provider-owned seams, while Docker/Traefik materialization, logs, monitoring, and hidden runtime-resource projection remain sample-local. |
